...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'R024 complete build/test gate failed.' }
    $jar = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.17.jar'
    ...
    $protected = @(& git diff --name-only 8aa4ae3 -- @protectedPaths | Where-Object { $_ -ne 'src/main/resources/Server/Item/RootInteractions/RPG/Root_RPG_Ability_Bridge.json' })
    ...
    $result = [ordered]@{
        capturedAtUtc=[DateTime]::UtcNow.ToString('o'); revision='R024'; version='0.0.17'; branch=(& git branch --show-current).Trim()
        baseline='8aa4ae3'; sourceCommit=(& git rev-parse HEAD).Trim()
    ...
    if ($failures -or $errors -or $skipped -or $protected.Count -or $shippedOverrides.Count) { throw 'R024 verification failed.' }
