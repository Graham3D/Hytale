...
$out=Join-Path $repo 'evidence/stage-13/cohort-an'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-am/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE revision=R032-AN enabled=true .*liveTest=true disposableWorldRequired=false connectedProof=false'){throw 'Live probe automatic registration missing'}
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingPresentationProbe|HealingProbePolicy|HealingTetherPresentation|SplineHealingParticleVisuals|HytaleSkillExecutionSystem)|ui/hud/RpgHud|commands/HealingProbeCommand|phase00/Phase00Plugin)[^/]*\.class|healing-probe-live-test\.txt|Server/Models/RPG/RPG_Heal_Path_Visible_AN\.json)$'){throw "Unscoped JAR change: $($e.FullName)"}
    ...
    $mr=[IO.StreamReader]::new($marker.Open());try{if($mr.ReadToEnd().Trim() -ne 'R032-AN'){throw 'Invalid live-test marker'}}finally{$mr.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AN-three-mods.zip'
...
[ordered]@{cohort='AN';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=0;initialFullRunFailures=0;
    finalReviewCorrection='Single visible model carrier proof; AM assets, spline and gameplay unchanged';
    ...
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AM -> AN -> AM PASS';
...
if(($suites|Measure-Object tests -Sum).Sum -ne 2276){throw 'Expected all 2276 retained-plus-new tests; do not package a partial suite'}
if((Get-FileHash (Join-Path $liveMods 'HyARPG.jar')).Hash -ne 'DA75E29BB572DDAE7639A9C8AF98A042332CDCDCD396B491C9D5C0A52C327237'){throw 'Live AM JAR changed during packaging-only task'}
...
if($nativeLog -notmatch 'RPG_HEAL_PARTICLE_PATH_NATIVE revision=R032-AN result=PASS' -or $nativeLog -notmatch 'RPG_HEAL_PARTICLE_PATH_ASSETS revision=R032-AM result=PASS'){throw 'AM production particle derivative and lifecycle audit missing'}
if($nativeLog -notmatch 'RPG_HEAL_VISIBLE_ANCHOR_NATIVE revision=R032-AN result=PASS exactlyOne=true persistent=true moved=true cleanup=true connectedProof=false'){throw 'AN visible carrier gate missing'}
