...
$rpgJar = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.5.jar'
$runDirectory = Join-Path $projectRoot 'run\stage-03-smoke'
...
Copy-Item -LiteralPath $rpgJar -Destination (Join-Path $modsDirectory 'HytaleRPG-0.0.5.jar') -Force
...
$summary = [ordered]@{
    ...
    rpgDiscovered = [bool]($plain -match 'InigmasGames:HytaleRPGPhase00Audit from path HytaleRPG-0\.0\.5\.jar')
    ...
    rpgSetup = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R012 version=0\.0\.5 hytale=0\.7\.0-pre\.1 stage=03 combatEnabled=true')
    stageReady = [bool]($plain -match 'RPG_STAGE03_READY revision=R012 skills=87 passives=66 schema=2 balance=rpg\.combat-kernel\.r011')
...
if (-not ($summary.serverVersionObserved -and $summary.rpgDiscovered -and $summary.canvasUiDiscovered -and
          ...
          $summary.rpgFailureObserved) { throw 'Stage 03 server smoke gate failed.' }
