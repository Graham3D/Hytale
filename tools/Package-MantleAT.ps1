...
$baseline=Join-Path $repo 'evidence/stage-13/cohort-as/artifacts/HyARPG.jar'
...
if((Hash $baseline) -ne '114AFD95E29A4DC49DBAE688EA8D0038680FF56AE1855A1F3AC649185A033E25'){throw 'AS rollback artifact mismatch'}
...
try{
    ...
    if($removed.Count){throw "Unexpected removal from cumulative AS build: $removed"}
    ...
    foreach($path in $oldEntries.Keys){
        if($path -match '^com/inigmasgames/hytalerpg/(input/|execution/hytale/(Healing|SplineHealing)|execution/projectile/)' -or
           $path -match '^Server/(Particles/RPG/RPG_Mantle_(Aura|Pulse)\.particlesystem|Entity/Effects/RPG/RPG_Mantle_(Pulse|Flash)\.json|Item/Items/Weapon/)' -or
           $path -match '^com/inigmasgames/hytalerpg/(execution/support/|combat/(resource/|cooldown/))' -or
    ...
    $r=[IO.StreamReader]::new($new.GetEntry('rpg-build.properties').Open());try{if($r.ReadToEnd() -notmatch 'rpg.revision=R032-AT'){throw 'Candidate identity is not AT'}}finally{$r.Dispose()}
...
$profileLine=($log -split "\r?\n" | Where-Object {$_ -match 'RPG_LIGHT_ATTACK_PROFILES \{' } | Select-Object -Last 1)
if(-not $profileLine){throw 'Missing native profile inventory'}
$profileLine.Substring($profileLine.IndexOf('{')) | Set-Content (Join-Path $out 'native-light-profiles.json') -Encoding utf8
...
$zip=Join-Path $out 'HyARPG-R032-AT-three-mods.zip'
...
$record=[ordered]@{status='IMPLEMENTED_AWAITING_CONNECTED_VERIFICATION';coverage='ADDITIVE_WEAPON_FIRE_ECHO';startingHead=(& git -C $repo rev-parse HEAD).Trim();revision='R032-AT';capturedAtUtc=[DateTime]::UtcNow.ToString('o');suites=$suites;totalTests=($suites|Measure-Object tests -Sum).Sum;jarHashes=$hashes;archiveSha256=(Hash $zip);rollbackArtifact=$baseline;rollbackSha256=(Hash $baseline);binaryRollback='AS -> AT -> AS PASS';entryDifferences=$diff;removedEntries=$removed;connectedVerified=$false;deployed=$false;pushed=$false}
if($Deploy){
    AssertStopped
    ...
    $pending=Join-Path $mods 'HyARPG.at.pending'
