...
$old=Join-Path $mods 'HytaleRPG-0.0.25.jar'
...
$expected='5629BDEA510ED1543DC43812A8E10EDE84744D5037ECEBDD7335369E380E0C77'
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AE artifact'}
if((Test-Path $target) -or (Get-FileHash $old).Hash -ne 'CE44364A24A42B3460523342AF32614CB0312057EE4B98CD2E0BBE71C442F38C'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.ae-staging'
...
Move-Item -LiteralPath $old -Destination (Join-Path $retired 'HytaleRPG-0.0.25.jar')
try{Move-Item -LiteralPath $staging -Destination $target}
catch{Move-Item -LiteralPath (Join-Path $retired 'HytaleRPG-0.0.25.jar') -Destination $old;throw}
if((Get-FileHash $target).Hash -ne $expected){throw 'Installed JAR hash mismatch'}
foreach($file in $inventory){
    if($file.path -eq 'mods\HytaleRPG-0.0.25.jar'){continue}
