...
$out=Join-Path $repo 'evidence/stage-13/cohort-ag'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-af/artifacts/HyARPG.jar'
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingParticleVisuals|HytaleSkillExecutionSystem|NativeProjectileSpawnAuditCommand)|execution/connection/(ConnectionRuntime|ConnectionWorldPort)|input/NativeSupportTetherAudit|ui/hud/RpgHud)[^/]*\.class|Server/Item/Items/Weapon/Staff/[^/]+\.json|Server/Models/RPG/RPG_Healing_Stream\.json|Server/Entity/Effects/RPG/RPG_Healing_Recipient\.json|rpg/presentation/staff-heads-ag\.json)$'){throw "Unscoped JAR change: $($e.FullName)"}
...
$zip=Join-Path $out 'HyARPG-R032-AG-three-mods.zip'
...
[ordered]@{cohort='AG';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=1;initialFullRunFailures=7;
    initialFailureReason='Icon updater correctly refused while isolated smoke Hytale process was running';
    targetedRerun='Stage13IconUpdaterTest, server stopped, 9 tests PASS, unchanged assertions';
    isolatedNativeParticleLifecycle='PASS';connectedVerified=$false;
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AF -> AG -> AF PASS';
