...
$out=Join-Path $repo 'evidence/stage-13/cohort-ae'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ad/artifacts/HytaleRPG-0.0.25.jar'
...
$owner=[IO.Compression.ZipFile]::OpenRead((Join-Path $liveMods 'HytaleRPG-0.0.25.jar'))
function JsonEntry($e){$r=[IO.StreamReader]::new($e.Open());try{$r.ReadToEnd()|ConvertFrom-Json|ConvertTo-Json -Depth 30 -Compress}finally{$r.Dispose()}}
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        $ownerIcon=$e.FullName -match '^Common/(UI/Custom/Icons|Icons/Items)/RPG/Skill(Blizzard|Healingbeam|Snipe|Whirlwind)\.png$'
        $ownerMapping=$e.FullName -match '^Server/Item/Items/RPG/Abilities/RPG_Ability_(Blizzard|Healing_Beam|Snipe|Whirlwind)\.json$'
        if($ownerIcon -and $h -ne (EntryHash $owner.GetEntry($e.FullName))){throw 'Owner icon bytes were not preserved'}
        if($ownerMapping -and (JsonEntry $e) -ne (JsonEntry $owner.GetEntry($e.FullName))){throw 'Owner item mapping/content was not preserved'}
        if(-not $ownerIcon -and -not $ownerMapping){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HytaleSkillExecutionSystem|NativeHealingBeamVisuals|NativeProjectileSpawnAuditCommand)|input/NativeSupportTetherAudit|phase00/Phase00Plugin|ui/(RpgUiProjectionService|hud/(CooldownSweep|RpgHud|RpgHudCoordinator)|model/SkillSlotView))[^/]*\.class|manifest\.json|rpg-build\.properties|Common/UI/Custom/(RpgCooldownSweep\.ui|Assets/RpgHud/CooldownMask\.png))$'){throw "Unscoped JAR change: $($e.FullName)"}
        }
...
}finally{$old.Dispose();$new.Dispose();$owner.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AE-three-mods.zip'
...
[ordered]@{cohort='AE';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=0;isolatedNativeBeamLifecycle='PASS';connectedVerified=$false;
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AD -> AE -> AD PASS';
