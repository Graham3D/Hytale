...
$runDirectory = Join-Path $projectRoot 'run\r018-smoke'; $mods = Join-Path $runDirectory 'mods'
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.11.jar') -Destination $resolved
...
try {
    ...
    if (-not $process.Start()) { throw 'Could not start R018 smoke server.' }
    $stdout = $process.StandardOutput.ReadToEndAsync(); $stderr = $process.StandardError.ReadToEndAsync()
    Start-Sleep -Seconds 25
    if (-not $process.HasExited) { $process.StandardInput.WriteLine('stop'); $process.StandardInput.Flush() }
    if (-not $process.WaitForExit(30000)) { $process.Kill($true); throw 'R018 smoke server timeout.' }
...
$summary = [ordered]@{
    ...
    rpgDiscovered = [bool]($plain -match 'HytaleRPG-0\.0\.11\.jar')
    rpgSetup = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R018 version=0\.0\.11 hytale=0\.7\.0-pre\.1 stage=05')
    ready = [bool]($plain -match 'RPG_STAGE05_READY revision=R018 .* schema=3')
...
if (-not ($summary.exactlyThreeMods -and $summary.rpgDiscovered -and $summary.rpgSetup -and
    ...
    $summary.cleanShutdown) -or $summary.failure) { throw 'R018 smoke gate failed.' }
