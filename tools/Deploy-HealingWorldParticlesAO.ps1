...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AO artifact'}
if((Get-FileHash $old).Hash -ne '83404F4B97F419092561FD606D4E81276EF8D4B0DF92951417B58D7FCF4CFC1A'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.ao-staging'
