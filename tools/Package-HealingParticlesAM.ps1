...
$baseline=Join-Path $repo 'evidence/stage-13/cohort-al/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE revision=R032-AM enabled=true .*liveTest=true disposableWorldRequired=false connectedProof=false'){throw 'Live probe automatic registration missing'}
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingPresentationProbe|HealingProbePolicy|NativeProjectileSpawnAuditCommand|HealingTetherPresentation|HealingParticlePath|SplineHealingParticleVisuals|HytaleSkillExecutionSystem)|ui/hud/RpgHud|input/NativeSupportTetherAudit|input/NativeHealingParticlePathAudit|commands/HealingProbeCommand|phase00/Phase00Plugin)[^/]*\.class|healing-probe-live-test\.txt|Server/Particles/RPG/HealingPath/RPG_Heal_Path_(?:Sparks|Glow|Plus)\.particlespawner|Server/Particles/RPG/HealingPath/RPG_Heal_Path_(?:Blips|Pulse)\.particlesystem|Server/Models/RPG/RPG_Heal_Path_(?:Blips|Pulse)\.json)$'){throw "Unscoped JAR change: $($e.FullName)"}
    ...
    $mr=[IO.StreamReader]::new($marker.Open());try{if($mr.ReadToEnd().Trim() -ne 'R032-AM'){throw 'Invalid live-test marker'}}finally{$mr.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AM-three-mods.zip'
...
[ordered]@{cohort='AM';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=2;fullValidationExitCode=0;initialFullRunFailures=0;
    finalReviewCorrection='Restored shipped Glow/Plus concurrency ceilings; first passing candidate retained in attempts/pre-final-pulse-cap and never deployed';
    isolatedNativeParticleLifecycle='PASS';visibilityGate='A_CONNECTED_UNVERIFIED';laterGates='CONNECTED_APPEARANCE_FLOW_CONTINUITY_CLEANUP_UNVERIFIED';connectedVerified=$false;
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AL -> AM -> AL PASS';
...
if(($suites|Measure-Object tests -Sum).Sum -ne 2274){throw 'Expected all 2274 retained-plus-new tests; do not package a partial suite'}
if((Get-FileHash (Join-Path $liveMods 'HyARPG.jar')).Hash -ne '899C80A31F7165456909EC33DD7BCBC0ECD1EE6843DF0120F2822B037BBAF87D'){throw 'Live AL JAR changed during packaging-only task'}
...
if($nativeLog -notmatch 'RPG_HEAL_PARTICLE_PATH_NATIVE revision=R032-AM result=PASS' -or $nativeLog -notmatch 'RPG_HEAL_PARTICLE_PATH_ASSETS revision=R032-AM result=PASS'){throw 'AM production particle derivative and lifecycle audit missing'}
