...
$runDirectory = Join-Path $projectRoot 'run\r021-smoke'
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.14.jar') -Destination $resolved
...
try {
    ...
    if (-not $process.Start()) { throw 'Could not start R021 smoke server.' }
    $stdout = $process.StandardOutput.ReadToEndAsync(); $stderr = $process.StandardError.ReadToEndAsync()
    Start-Sleep -Seconds 30
    if (-not $process.HasExited) { $process.StandardInput.WriteLine('stop'); $process.StandardInput.Flush() }
    if (-not $process.WaitForExit(30000)) { $process.Kill($true); throw 'R021 smoke server timeout.' }
...
$summary = [ordered]@{
    ...
    rpgDiscovered = [bool]($plain -match 'HytaleRPG-0\.0\.14\.jar')
    rpgSetup = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R021 version=0\.0\.14 hytale=0\.7\.0-pre\.1 stage=05')
    ready = [bool]($plain -match 'RPG_STAGE05_READY revision=R021 .* nativeAbility4=NATIVE_ABILITY4_UNAVAILABLE .* abilityHud=NATIVE_HYTALE_ONLY')
...
if (-not ($summary.exactlyThreeMods -and $summary.rpgDiscovered -and $summary.rpgSetup -and $summary.ready -and
    $summary.pluginEnabled -and $summary.managerStarted -and $summary.cleanShutdown) -or
    $summary.nativeAbilityAssetsRejected -or $summary.failure) { throw 'R021 smoke gate failed.' }
