...
$runDirectory = Join-Path $projectRoot 'run\r023-smoke'
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.16.jar') -Destination $resolved
...
try {
    ...
    if (-not $process.Start()) { throw 'Could not start R023 smoke server.' }
    $stdout = $process.StandardOutput.ReadToEndAsync(); $stderr = $process.StandardError.ReadToEndAsync()
    Start-Sleep -Seconds 30
    if (-not $process.HasExited) { $process.StandardInput.WriteLine('stop'); $process.StandardInput.Flush() }
    if (-not $process.WaitForExit(30000)) { $process.Kill($true); throw 'R023 smoke server timeout.' }
...
$summary = [ordered]@{
    ...
    rpgDiscovered = [bool]($plain -match 'HytaleRPG-0\.0\.16\.jar')
    rpgSetup = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R023 version=0\.0\.16 hytale=0\.7\.0-pre\.1 stage=05')
    ready = [bool]($plain -match 'RPG_STAGE05_READY revision=R023 .* abilityHud=NATIVE_HYTALE_ONLY')
    packagedRootResolved = [bool]($plain -match 'RPG_NATIVE_BRIDGE_AUDIT revision=R023 root=Root_RPG_Ability_Bridge exists=true operations=1 operation=FirstClickInteraction waitFor=Client operationRemote=true rootRemote=true effectFree=true result=PASS')
    shippedRuneResolved = [bool]($plain -match 'RPG_NATIVE_RUNE_CONTROL_AUDIT revision=R023 item=Rune_Fireball root=Root_Ability_Fireball .* result=PASS connectedProof=false')
