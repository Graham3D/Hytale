...
$sourceJar = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.6.jar'
$targetJar = Join-Path $SaveModsDirectory 'HytaleRPG-0.0.6.jar'
$evidenceDirectory = Join-Path $projectRoot 'evidence\stage-04\R013'
$rollbackDirectory = Join-Path $evidenceDirectory 'rollback'

if (-not (Test-Path -LiteralPath $sourceJar -PathType Leaf)) { throw "Build Stage 04 first: $sourceJar" }
...
$expectedNames = @('CanvasUI-0.1.0.jar', 'HYTALEDEVLIB-0.5.0.jar', 'HytaleRPG-0.0.6.jar')
...
$result = [ordered]@{
    ...
    canvasUiSha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $SaveModsDirectory 'CanvasUI-0.1.0.jar')).Hash
    hytaleDevLibSha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $SaveModsDirectory 'HYTALEDEVLIB-0.5.0.jar')).Hash
    ...
    rollback = 'Stop the RPG world, remove HytaleRPG-0.0.6.jar, and restore HytaleRPG-0.0.5.jar from evidence/stage-04/R013/rollback into Saves/RPG/mods. Player schema remains v2.'
}
