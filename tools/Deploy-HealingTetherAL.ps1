...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AL artifact'}
if((Get-FileHash $old).Hash -ne '3F0A570703F4BEE8C86773E09D1F5FC985A7FAFB6DD42AD0584DBC4AAF0A64AA'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.al-staging'
