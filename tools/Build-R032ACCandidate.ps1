...
$base=Join-Path $repo 'evidence\stage-13\cohort-ab\artifacts\HytaleRPG-0.0.25.jar'
$built=Join-Path $repo 'build\libs\HytaleRPG-0.0.25.jar'
$directory=Join-Path $repo 'build\candidate-ac'
...
try{
  foreach($entry in $source.Entries){
    $name=$entry.FullName
    $allowed=$name -match '^com/inigmasgames/hytalerpg/execution/connection/(ConnectionRuntime|ConnectionWorldPort)(\$[^/]*)?\.class$' -or
      $name -match '^com/inigmasgames/hytalerpg/execution/hytale/(HytaleSkillExecutionSystem|NativeBeamTransform|ElasticBeamTether|NativeHealingBeamVisuals)(\$[^/]*)?\.class$' -or
      $name -eq 'com/inigmasgames/hytalerpg/input/NativeSupportTetherAudit.class' -or
      $name -eq 'Server/Entity/Beams/RPG_Healing.json'
...
if($changed.Count -lt 8 -or $changed -notcontains 'Server/Entity/Beams/RPG_Healing.json' -or
   $changed -notcontains 'com/inigmasgames/hytalerpg/execution/hytale/NativeHealingBeamVisuals.class'){
    throw 'AC candidate is missing required native-beam entries'
}
[ordered]@{baseline=(Get-FileHash $base).Hash;jarSha256=(Get-FileHash $candidate).Hash;changedEntries=@($changed);otherABEntriesByteIdentical=$true;deployed=$false} |
