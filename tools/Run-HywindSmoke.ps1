...
$summary = [ordered]@{
    ...
    hywindSetup = [bool]($plain -match 'HYWIND_SETUP version=0\.1\.0-merge\.8 revision=R053 .*TAVERNS')
    ...
    canvasOwned = [bool]($plain -match 'CANVASUI_SETUP revision=R053 .*owner=HYWIND')
    rpgOwned = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R053 version=0\.1\.0-merge\.8 hytale=0\.7\.0-pre\.3\.1 stage=13')
    ...
    hywindStarted = [bool]($plain -match 'HYWIND_STARTED version=0\.1\.0-merge\.8 revision=R053')
    hywindShutdown = [bool]($plain -match 'HYWIND_SHUTDOWN version=0\.1\.0-merge\.8 revision=R053')
