...
$summary = [ordered]@{
    ...
    hywindSetup = [bool]($plain -match 'HYWIND_SETUP version=0\.1\.0-merge\.22 revision=R067 .*TAVERNS')
    ...
    canvasOwned = [bool]($plain -match 'CANVASUI_SETUP revision=R067 .*owner=HYWIND')
    rpgOwned = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R067 version=0\.1\.0-merge\.22 hytale=0\.7\.0-pre\.3\.1 stage=13')
    ...
    hywindStarted = [bool]($plain -match 'HYWIND_STARTED version=0\.1\.0-merge\.22 revision=R067')
    hywindShutdown = [bool]($plain -match 'HYWIND_SHUTDOWN version=0\.1\.0-merge\.22 revision=R067')
