...
$expectedSupport = 16
$expectedSummons = switch($Cohort){'a'{1};'b'{2};'c'{4};'d'{6};'e'{7};'f'{8};'g'{9};default{throw 'Cohort is not configured'}}
...
$summary = [ordered]@{
    ...
    batchRolesResolved = $Cohort -notin @('c','d','e','f','g') -or [bool]($plain -match 'RPG_STAGE10_BATCH_ROLES count=3 result=PASS connectedProof=false')
    decoyRoleResolved = $Cohort -notin @('e','f','g') -or [bool]($plain -match 'RPG_STAGE10_DECOY_ROLE appearance=Mannequin attacks=0 result=PASS connectedProof=false')
