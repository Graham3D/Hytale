...
$out=Join-Path $repo 'evidence/stage-13/cohort-aq'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ap/artifacts/HyARPG.jar'
...
if((Hash $baseline) -ne 'E11720D6AC54BFCD7FD294308E908E124967EFAE39B4E935C016AD2AA24CD285'){throw 'AP rollback artifact mismatch'}
...
try{
    ...
    if($removed.Count){throw "Unexpected removal from cumulative AP build: $removed"}
    ...
    $r=[IO.StreamReader]::new($new.GetEntry('rpg-build.properties').Open());try{if($r.ReadToEnd() -notmatch 'rpg.revision=R032-AQ'){throw 'Candidate identity is not AQ'}}finally{$r.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AQ-three-mods.zip'
...
$record=[ordered]@{status='IMPLEMENTED_AWAITING_CONNECTED_VERIFICATION';coverage='LIMITED_FLAME_LONGSWORD_PRIMARY';startingHead=(& git -C $repo rev-parse HEAD).Trim();revision='R032-AQ';capturedAtUtc=[DateTime]::UtcNow.ToString('o');suites=$suites;totalTests=($suites|Measure-Object tests -Sum).Sum;jarHashes=$hashes;archiveSha256=(Hash $zip);rollbackArtifact=$baseline;rollbackSha256=(Hash $baseline);binaryRollback='AP -> AQ -> AP PASS';entryDifferences=$diff;removedEntries=$removed;connectedVerified=$false;deployed=$false;pushed=$false}
if($Deploy){
    AssertStopped
    ...
    $pending=Join-Path $mods 'HyARPG.aq.pending'
