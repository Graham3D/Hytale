...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AN artifact'}
if((Get-FileHash $old).Hash -ne 'DA75E29BB572DDAE7639A9C8AF98A042332CDCDCD396B491C9D5C0A52C327237'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.an-staging'
