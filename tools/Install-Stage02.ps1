...
$sourceJar = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.3.jar'
$targetJar = Join-Path $SaveModsDirectory 'HytaleRPG-0.0.3.jar'
...
$expectedNames = @('CanvasUI-0.1.0.jar', 'HYTALEDEVLIB-0.5.0.jar', 'HytaleRPG-0.0.3.jar')
...
$result = [ordered]@{
    ...
    rollback = 'Stop the RPG world, remove HytaleRPG-0.0.3.jar, and restore the retained R009 HytaleRPG-0.0.2.jar into Saves/RPG/mods. Player schema remains v2.'
}
