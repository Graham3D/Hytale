...
$source = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.11.jar'
$evidence = Join-Path $projectRoot 'evidence\corrections\R018'; $rollback = Join-Path $evidence 'rollback'
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Build R018 first: $source" }
...
$target = Join-Path $mods 'HytaleRPG-0.0.11.jar'; Copy-Item -LiteralPath $source -Destination $target -Force
...
$expected = @('CanvasUI-0.1.0.jar','HYTALEDEVLIB-0.5.0.jar','HytaleRPG-0.0.11.jar')
if ((Compare-Object $expected @($jars.Name)).Count -ne 0) { throw "Expected exactly three mods; found $($jars.Name -join ', ')" }
$result = [ordered]@{
    installedAtUtc = [DateTime]::UtcNow.ToString('o'); revision = 'R018'; version = '0.0.11'; schema = 3
    ...
    rollback = 'Stop the RPG world, remove HytaleRPG-0.0.11.jar, and restore HytaleRPG-0.0.10.jar from this rollback directory.'
}
