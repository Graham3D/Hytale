...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AG artifact'}
if((Get-FileHash $old).Hash -ne '1C485513F33E760EA3DE1AC8C52CD597D9CC54EBD2EF7B3C0C35C6922DA34134'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.ag-staging'
