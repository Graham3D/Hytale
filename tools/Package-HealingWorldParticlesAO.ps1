...
$out=Join-Path $repo 'evidence/stage-13/cohort-ao'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-an/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE revision=R032-AO enabled=true .*liveTest=true disposableWorldRequired=false connectedProof=false'){throw 'Live probe automatic registration missing'}
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingPresentationProbe|HealingProbePolicy|HealingTetherPresentation|HealingWorldParticleFrame|SplineHealingParticleVisuals|HytaleSkillExecutionSystem)|ui/hud/RpgHud|input/NativeHealingParticlePathAudit|commands/HealingProbeCommand|phase00/Phase00Plugin)[^/]*\.class|healing-probe-live-test\.txt|Server/Particles/RPG/HealingWorld/RPG_Heal_World_(?:Sparks|Glow|Plus)\.particlespawner|Server/Particles/RPG/HealingWorld/RPG_Heal_World_(?:Blips|Pulse)\.particlesystem)$'){throw "Unscoped JAR change: $($e.FullName)"}
        @{entry=$e.FullName;before=$oldEntries[$e.FullName];after=$h}
    };$oldEntries.Remove($e.FullName)}})
    if($oldEntries.Count -ne 1 -or -not $oldEntries.ContainsKey('Server/Models/RPG/RPG_Heal_Path_Visible_AN.json')){throw 'Only the retired AN visible model may be removed'}
    $marker=$new.GetEntry('healing-probe-live-test.txt');if(-not $marker){throw 'Live-test opt-in marker missing'}
    $mr=[IO.StreamReader]::new($marker.Open());try{if($mr.ReadToEnd().Trim() -ne 'R032-AO'){throw 'Invalid live-test marker'}}finally{$mr.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AO-three-mods.zip'
...
[ordered]@{cohort='AO';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=0;initialFullRunFailures=0;
    finalReviewCorrection='Production world-space particle renderer; finite appearance-exact bursts, scoped viewers, no tether entities';
    ...
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AN -> AO -> AN PASS';
    removedEntry='Server/Models/RPG/RPG_Heal_Path_Visible_AN.json';unchangedEntriesPreserved=$true;entryDifferences=$diff;internalIdentityPreserved=$true;liveDeploymentPerformed=$false
}|ConvertTo-Json -Depth 8|Set-Content (Join-Path $out 'package-validation.json') -Encoding utf8
if(($suites|Measure-Object tests -Sum).Sum -ne 2284){throw 'Expected all 2284 retained-plus-new tests; do not package a partial suite'}
if((Get-FileHash (Join-Path $liveMods 'HyARPG.jar')).Hash -ne '83404F4B97F419092561FD606D4E81276EF8D4B0DF92951417B58D7FCF4CFC1A'){throw 'Live AN JAR changed during packaging-only task'}
...
if($nativeLog -notmatch 'RPG_HEAL_WORLD_PARTICLE_NATIVE revision=R032-AO result=PASS' -or $nativeLog -notmatch 'RPG_HEAL_PARTICLE_PATH_ASSETS revision=R032-AM result=PASS'){throw 'AM production particle derivative and lifecycle audit missing'}
