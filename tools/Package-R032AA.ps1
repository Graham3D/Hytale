...
$candidate=Join-Path $repo 'build\candidate-aa\HytaleRPG-0.0.25.jar'
...
foreach($name in $names){
    $source=if($name -eq $names[0]){$candidate}else{Join-Path $repo "evidence\stage-13\cohort-z\artifacts\$name"}
...
Copy-Item -LiteralPath (Join-Path $repo 'build\candidate-aa\differential.json') -Destination $evidence -Force
$counts=@()
...
$archive=Join-Path $evidence 'HytaleRPG-R032-AA-three-mods.zip'
...
$rollback=Join-Path $repo 'build\candidate-aa\rollback-check.jar'
$baseline=Join-Path $repo 'evidence\stage-13\cohort-z\artifacts\HytaleRPG-0.0.25.jar'
