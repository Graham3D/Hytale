...
$run = Join-Path $root 'run\canvasui-smoke'; $mods = Join-Path $run 'mods'; $evidence = Join-Path $root 'evidence\canvas-ui\R003'
...
$summary = [ordered]@{
    ...
    canvasSetup = [bool]($plain -match 'CANVASUI_SETUP revision=R003 version=0\.1\.0 hytale=0\.7\.0-pre\.1')
    ...
    demoSetup = [bool]($plain -match 'CANVASUI_DEMO_SETUP revision=R003')
