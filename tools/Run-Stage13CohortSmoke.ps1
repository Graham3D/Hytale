...
if($NativeProjectileSpawnAudit){
    ...
    [ordered]@{result='PASS';jarSha256=$summary.jarSha256;startedUtc=$startedUtc.ToString('o');nativeApi='ProjectileModule.spawnProjectile';
        ...
        sameTickAdvanceAndRestingExpiry=($Cohort -in @('p','q','r','s','t','u','v'));
...
if($Cohort -eq 'v' -and -not $summary.supportTetherAssetsResolved){throw 'V held/effect/particle native asset resolution failed'}
...
if($Cohort -ne 'a'){
    ...
    if($Cohort -in @('c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v') -and ($audit.resolvedConfigs -ne 19 -or $audit.equipment.Weapon_Gun_Blunderbuss.basicPower -ne 200 -or
        $audit.nativeChargedBow.speed -ne 85 -or $audit.nativeChargedBow.gravity -ne 25 -or $audit.nativeChargedBow.radius -ne .075 -or
        $audit.snipeActivationGate -ne $(if($Cohort -in @('t','u','v')){''}else{'NATIVE_BOW_MAX_RANGE_UNVERIFIED'}))){throw 'Cohort C native source/capability audit mismatch'}
    if($Cohort -in @('t','u','v') -and ($audit.nativeChargedBow.maximumRange -ne 'RPG_AUTHORED_48M_NOT_NATIVE_MAXIMUM' -or
    ...
    if($Cohort -in @('u','v') -and ($audit.nativeChargedBow.snipeGravity -ne 0 -or
...
if($Cohort -in @('d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v')){
...
if($Cohort -in @('e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v')){
