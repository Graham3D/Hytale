...
$source = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.13.jar'
$evidence = Join-Path $projectRoot 'evidence\corrections\R020'; $rollback = Join-Path $evidence 'rollback'
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Build R020 first: $source" }
...
$result = [ordered]@{
    installedAtUtc = [DateTime]::UtcNow.ToString('o'); revision = 'R020'; version = '0.0.13'; schema = 3
    ...
    rollback = 'Stop the RPG world, remove HytaleRPG-0.0.13.jar, and restore HytaleRPG-0.0.12.jar from this rollback directory.'
}
