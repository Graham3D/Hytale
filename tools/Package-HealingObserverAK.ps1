...
$out=Join-Path $repo 'evidence/stage-13/cohort-ak'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-aj-live/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE revision=R032-AK enabled=true .*liveTest=true disposableWorldRequired=false connectedProof=false'){throw 'Live probe automatic registration missing'}
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingPresentationProbe|HealingProbePolicy|HealingParticleVisuals|NativeProjectileSpawnAuditCommand)|commands/HealingProbeCommand|phase00/Phase00Plugin)[^/]*\.class|healing-probe-live-test\.txt)$'){throw "Unscoped JAR change: $($e.FullName)"}
    ...
    $mr=[IO.StreamReader]::new($marker.Open());try{if($mr.ReadToEnd().Trim() -ne 'R032-AK'){throw 'Invalid live-test marker'}}finally{$mr.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AK-three-mods.zip'
...
[ordered]@{cohort='AK';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    ...
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AJ-LIVE -> AK -> AJ-LIVE PASS';
...
if(($suites|Measure-Object tests -Sum).Sum -ne 2261){throw 'Expected all 2261 retained-plus-new tests; do not package a partial suite'}
if((Get-FileHash (Join-Path $liveMods 'HyARPG.jar')).Hash -ne '774BD5844BEE9D00C0941A827A4D4F3FF1E87E39C8F1F25D2BCA3AB75200C79E'){throw 'Live AJ-LIVE JAR changed during packaging-only task'}
