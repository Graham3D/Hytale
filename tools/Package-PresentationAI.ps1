...
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ah/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE_NATIVE result=PASS models=2 effects=5 onceAndOverwrite=true nativeEffectQueue=true nonConsumingRead=true order=.* connectedProof=false'){throw 'AI probe native gate missing'}
...
foreach($suite in @('build/test-results/test','build/test-results/nativeControlTest','canvas-ui/build/test-results/test')){
    ...
    $suiteSource=Join-Path $out ('full-run/'+$suite.Replace('/','-'))
    if(-not(Test-Path $target)){New-Item -ItemType Directory -Force $target|Out-Null;Get-ChildItem $suiteSource -Filter 'TEST-*.xml'|Copy-Item -Destination $target}
    # The one full run found missing probe tick instrumentation. Preserve it under full-run;
    # overlay only executed focused-final test classes after fixing the instrumentation, not the test.
    if($suite -eq 'build/test-results/test'){
        Get-ChildItem (Join-Path $repo $suite) -Filter 'TEST-*.xml'|Copy-Item -Destination $target -Force
    }
    $suiteSource=$target
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingParticleVisuals|HealingPresentationProbe|HealingProbePolicy|NativeProjectileSpawnAuditCommand)|commands/HealingProbeCommand|phase00/Phase00Plugin|ui/hud/RpgHud)[^/]*\.class|Server/Models/RPG/RPG_Probe_Healing_Visible\.json|Server/Entity/Effects/RPG/RPG_Probe_Healing_(Recipient|Staff_[^/]+)\.json)$'){throw "Unscoped JAR change: $($e.FullName)"}
...
$zip=Join-Path $out 'HyARPG-R032-AI-three-mods.zip'
...
[ordered]@{cohort='AI';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=1;initialFullRunFailures=1;
    initialFailureReason='New opt-in probe ticks lacked required NativeRpgTickMetrics spans; production callback instrumentation corrected; assertion unchanged';
    targetedRerun='17 tests PASS: NativeTickMetrics plus AI probe and AH/AG retained presentation tests';
    ...
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AH -> AI -> AH PASS';
...
if(($suites|Measure-Object tests -Sum).Sum -ne 2252){throw 'Expected all 2252 retained-plus-new tests; do not package a partial suite'}
