...
$out=Join-Path $repo 'evidence/stage-13/cohort-aj'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ai/artifacts/HyARPG.jar'
...
if($nativeLog -notmatch 'RPG_HEAL_PROBE_FIXTURE result=PASS' -or $nativeLog -notmatch 'RPG_HEAL_PROBE_MODE_PATHS result=PASS standalone=4 recipient=2 sameCreateUpdateCleanup=true noStandaloneNpc=true connectedProof=false'){throw 'AJ actual mode/fixture gate missing'}
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingPresentationProbe|HealingProbePolicy)|commands/HealingProbeCommand|phase00/Phase00Plugin)[^/]*\.class)$'){throw "Unscoped JAR change: $($e.FullName)"}
...
$zip=Join-Path $out 'HyARPG-R032-AJ-three-mods.zip'
...
[ordered]@{cohort='AJ';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    ...
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AI -> AJ -> AI PASS';
...
if(($suites|Measure-Object tests -Sum).Sum -ne 2255){throw 'Expected all 2255 retained-plus-new tests; do not package a partial suite'}
