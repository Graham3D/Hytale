...
$out=Join-Path $repo 'evidence/stage-13/cohort-as'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ar/artifacts/HyARPG.jar'
...
if((Hash $baseline) -ne 'D424F051F55F9B61F89A84B2C8858192C42A55992A72419E513D79E4C263659D'){throw 'AR rollback artifact mismatch'}
...
try{
    ...
    if($removed.Count){throw "Unexpected removal from cumulative AR build: $removed"}
    ...
    $r=[IO.StreamReader]::new($new.GetEntry('rpg-build.properties').Open());try{if($r.ReadToEnd() -notmatch 'rpg.revision=R032-AS'){throw 'Candidate identity is not AR'}}finally{$r.Dispose()}
...
$zip=Join-Path $out 'HyARPG-R032-AS-three-mods.zip'
...
$record=[ordered]@{status='IMPLEMENTED_AWAITING_CONNECTED_VERIFICATION';coverage='PROFILE_BASED_RPG_WEAPON_STRIKES';startingHead=(& git -C $repo rev-parse HEAD).Trim();revision='R032-AS';capturedAtUtc=[DateTime]::UtcNow.ToString('o');suites=$suites;totalTests=($suites|Measure-Object tests -Sum).Sum;jarHashes=$hashes;archiveSha256=(Hash $zip);rollbackArtifact=$baseline;rollbackSha256=(Hash $baseline);binaryRollback='AR -> AS -> AR PASS';entryDifferences=$diff;removedEntries=$removed;connectedVerified=$false;deployed=$false;pushed=$false}
if($Deploy){
    AssertStopped
    ...
    $pending=Join-Path $mods 'HyARPG.as.pending'
