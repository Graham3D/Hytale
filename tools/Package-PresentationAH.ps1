...
$out=Join-Path $repo 'evidence/stage-13/cohort-ah'
$candidate=Join-Path $repo 'build/libs/HyARPG.jar'
$baseline=Join-Path $repo 'evidence/stage-13/cohort-ag/artifacts/HyARPG.jar'
...
try{
    ...
    $diff=@(foreach($e in $new.Entries){if($e.Name){$h=EntryHash $e;if(-not $oldEntries.ContainsKey($e.FullName) -or $h -ne $oldEntries[$e.FullName]){
        if($e.FullName -notmatch '^(com/inigmasgames/hytalerpg/(execution/hytale/(HealingParticleVisuals|HytaleSkillExecutionSystem|NativeProjectileSpawnAuditCommand|NativeManaReplicationAudit)|execution/connection/(ConnectionRuntime|ConnectionWorldPort)|input/NativeSupportTetherAudit|ui/hud/RpgHud)[^/]*\.class|Server/Item/Items/Weapon/Staff/[^/]+\.json|Server/Models/RPG/RPG_Healing_Stream\.json|Server/Entity/Effects/RPG/RPG_Healing_(Recipient|Staff_[^/]+)\.json|rpg/presentation/staff-heads-ag\.json)$'){throw "Unscoped JAR change: $($e.FullName)"}
...
$zip=Join-Path $out 'HyARPG-R032-AH-three-mods.zip'
...
[ordered]@{cohort='AH';startingHead=(& git -C $repo rev-parse HEAD).Trim();builtAtUtc=[DateTime]::UtcNow.ToString('o');
    suites=$suites;fullValidationRuns=1;fullValidationExitCode=0;initialFullRunFailures=0;
    initialFailureReason='None';
    targetedRerun='Not required';
    isolatedNativeParticleLifecycle='PASS';connectedVerified=$false;
    jarHashes=$hashes;archiveSha256=(Get-FileHash $zip).Hash;archiveEntriesVerified=$true;binaryRollback='AG -> AH -> AG PASS';
