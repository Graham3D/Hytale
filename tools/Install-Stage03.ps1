...
$sourceJar = Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.5.jar'
$targetJar = Join-Path $SaveModsDirectory 'HytaleRPG-0.0.5.jar'
$evidenceDirectory = Join-Path $projectRoot 'evidence\stage-03\R012'
$rollbackDirectory = Join-Path $evidenceDirectory 'rollback'

if (-not (Test-Path -LiteralPath $sourceJar -PathType Leaf)) { throw "Build Stage 03 first: $sourceJar" }
...
$expectedNames = @('CanvasUI-0.1.0.jar', 'HYTALEDEVLIB-0.5.0.jar', 'HytaleRPG-0.0.5.jar')
...
$canvasJar = Join-Path $SaveModsDirectory 'CanvasUI-0.1.0.jar'
$result = [ordered]@{
