[CmdletBinding()]
param([ValidateSet('a','b','c','d','e','f','g','h','i','j')][string]$Cohort='a')
$ErrorActionPreference='Stop'
$closureRoot=(Resolve-Path "$PSScriptRoot\..").Path
$closureOut=Join-Path $closureRoot "evidence\stage-13\cohort-$Cohort"
$expectedTests=@{a=815;b=1251;c=1298;d=1329;e=1378;f=1886;g=1913;h=1963;i=2040;j=2053}[$Cohort]
$expectedRuntime=@{a=66;b=72;c=78;d=83;e=86;f=87;g=87;h=87;i=87;j=87}[$Cohort]
$expectedPlan=@{a=36;b=37;c=38;d=39;e=40;f=41;g=41;h=41;i=41;j=41}[$Cohort]
...
if($Cohort -in @('e','f','g','h','i','j')){$testDirectories+='canvas-ui\build\test-results\test'}
...
if($Cohort -eq 'j'){
    & "$PSScriptRoot\Test-Stage13HandoffScope.ps1" -Tests $closureTests
}
...
if($Cohort -in @('e','f','g','h','i','j')){
    ...
    if($Cohort -in @('f','g','h','i','j')){$allowed+=@('src/main/java/com/inigmasgames/hytalerpg/ui/CharacterXpProjectionService.java','src/main/java/com/inigmasgames/hytalerpg/ui/trace/RpgUiTraceService.java')}
...
if($Cohort -eq 'j'){$changed=@($changed|Where-Object {$_ -ne 'src/main/java/com/inigmasgames/hytalerpg/ui/hud/RpgHudTickSystem.java'})}
...
try{
    ...
    if($Cohort -ne 'a'){
        ...
        if($Cohort -in @('f','g','h','i','j')){$required+=@('rpg/runtime/stage-13-stance-cohort-f.json','Server/Item/Items/RPG/Abilities/RPG_Ability_Frenzy.json')}
...
if($Cohort -in @('c','d','e','f','g','h','i','j')){
    ...
    if($Cohort -in @('d','e','f','g','h','i','j')){$gates+='NATIVE_GUARD_HELD_ITEM_RELEASE_ROUTE_UNVERIFIED'}
    if($Cohort -in @('f','g','h','i','j')){$gates+='NATIVE_PER_ACTOR_BASIC_ATTACK_CADENCE_UNVERIFIED'}
...
if($Cohort -in @('f','g','h','i','j')){
    ...
    if($Cohort -in @('h','i','j')){foreach($name in @('group-crash-matrix.json','group-sparse-load.json')){Copy-Item -LiteralPath (Join-Path $closureRoot "build\stage13-hardening\$name") -Destination (Join-Path $hardening $name) -Force}}
}
...
[ordered]@{capturedAtUtc=[DateTime]::UtcNow.ToString('o');stage=13;cohort=$Cohort;revision='R032';version='0.0.25';
    status=if($Cohort -eq 'j'){'LOCAL_CORRECTION_COMPLETE_PENDING_CONNECTED_QA'}else{'IMPLEMENTATION_IN_PROGRESS'};cohortLocalGate='PASS';stageClosure=if($Cohort -eq 'j'){'BLOCKED_NATIVE_TICK_NOT_MEASURED_AND_NATIVE_INTEGRATION'}elseif($Cohort -in @('f','g','h','i','j')){'BLOCKED_DURABLE_CONTRIBUTION_LATENCY_AND_NATIVE_INTEGRATION'}else{'NOT_COMPLETE'};connectedGate='UNVERIFIED';
    tests=$closureCount;failures=0;errors=0;skipped=0;regressionScope=if($Cohort -in @('e','f','g','h','i','j')){'COMPLETE_RETAINED_INTERMEDIATE_NOT_STAGE_CLOSURE'}else{'TARGETED_INTERMEDIATE_NOT_STAGE_CLOSURE'};
    ...
    liveDeploymentPerformed=$false;nativeCastingFixed=$false;stage13RollbackDrill=if($Cohort -eq 'j'){'ACTUAL_STAGE_I_ESCROW_AND_REWARD_COORDINATED_COPY_PASS_NOT_NATIVE_WORLD_PROOF'}elseif($Cohort -in @('f','g','h','i','j')){'ACTUAL_ARCHIVED_H_COPIED_CHECKPOINT_PASS_NOT_FINAL_RC'}else{'REQUIRED_AT_CLOSURE_AND_RELEASE_CANDIDATE'};
    explicitRuntimeGates=if($Cohort -in @('c','d','e','f','g','h','i','j')){$gates}else{@('BONE_CAGE_NATIVE_ENEMY_ONLY_COLLISION_UNVERIFIED')};
