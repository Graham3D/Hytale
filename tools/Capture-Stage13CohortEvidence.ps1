...
$expectedTests=@{a=815;b=1251;c=1298;d=1329;e=1378;f=1886;g=1913}[$Cohort]
$expectedRuntime=@{a=66;b=72;c=78;d=83;e=86;f=87;g=87}[$Cohort]
$expectedPlan=@{a=36;b=37;c=38;d=39;e=40;f=41;g=41}[$Cohort]
...
if($Cohort -in @('e','f','g')){$testDirectories+='canvas-ui\build\test-results\test'}
...
if($Cohort -in @('e','f','g')){
    ...
    if($Cohort -in @('f','g')){$allowed+=@('src/main/java/com/inigmasgames/hytalerpg/ui/CharacterXpProjectionService.java','src/main/java/com/inigmasgames/hytalerpg/ui/trace/RpgUiTraceService.java')}
...
try{
    ...
    if($Cohort -ne 'a'){
        ...
        if($Cohort -in @('f','g')){$required+=@('rpg/runtime/stage-13-stance-cohort-f.json','Server/Item/Items/RPG/Abilities/RPG_Ability_Frenzy.json')}
...
if($Cohort -in @('c','d','e','f','g')){
    ...
    if($Cohort -in @('d','e','f','g')){$gates+='NATIVE_GUARD_HELD_ITEM_RELEASE_ROUTE_UNVERIFIED'}
    if($Cohort -in @('f','g')){$gates+='NATIVE_PER_ACTOR_BASIC_ATTACK_CADENCE_UNVERIFIED'}
...
[ordered]@{capturedAtUtc=[DateTime]::UtcNow.ToString('o');stage=13;cohort=$Cohort;revision='R032';version='0.0.25';
    status='IMPLEMENTATION_IN_PROGRESS';cohortLocalGate='PASS';stageClosure=if($Cohort -in @('f','g')){'BLOCKED_DURABLE_CONTRIBUTION_LATENCY_AND_NATIVE_INTEGRATION'}else{'NOT_COMPLETE'};connectedGate='UNVERIFIED';
    tests=$closureCount;failures=0;errors=0;skipped=0;regressionScope=if($Cohort -in @('e','f','g')){'COMPLETE_RETAINED_INTERMEDIATE_NOT_STAGE_CLOSURE'}else{'TARGETED_INTERMEDIATE_NOT_STAGE_CLOSURE'};
    ...
    liveDeploymentPerformed=$false;nativeCastingFixed=$false;stage13RollbackDrill=if($Cohort -in @('f','g')){'ACTUAL_ARCHIVED_H_COPIED_CHECKPOINT_PASS_NOT_FINAL_RC'}else{'REQUIRED_AT_CLOSURE_AND_RELEASE_CANDIDATE'};
    explicitRuntimeGates=if($Cohort -in @('c','d','e','f','g')){$gates}else{@('BONE_CAGE_NATIVE_ENEMY_ONLY_COLLISION_UNVERIFIED')};
