[CmdletBinding()]
param([ValidateSet('a','b','c','d','e','f','g','h','i','j','k')][string]$Cohort='a')
$ErrorActionPreference='Stop'
$closureRoot=(Resolve-Path "$PSScriptRoot\..").Path
$closureOut=Join-Path $closureRoot "evidence\stage-13\cohort-$Cohort"
$expectedTests=@{a=815;b=1251;c=1298;d=1329;e=1378;f=1886;g=1913;h=1963;i=2040;j=2053;k=2087}[$Cohort]
$expectedRuntime=@{a=66;b=72;c=78;d=83;e=86;f=87;g=87;h=87;i=87;j=87;k=87}[$Cohort]
$expectedPlan=@{a=36;b=37;c=38;d=39;e=40;f=41;g=41;h=41;i=41;j=41;k=41}[$Cohort]
...
if($Cohort -in @('e','f','g','h','i','j','k')){$testDirectories+='canvas-ui\build\test-results\test'}
...
if($Cohort -eq 'i'){
    $retainedH=Get-Content -Raw -LiteralPath (Join-Path $closureRoot 'evidence\stage-13\cohort-h\test-results.json')|ConvertFrom-Json
    foreach($baseline in $retainedH){
        $current=@($closureTests|Where-Object {$_.name -eq $baseline.name})
        if($current.Count -ne 1 -or $current[0].tests -lt $baseline.tests){throw "Retained H suite reduced: $($baseline.name)"}
        foreach($case in $baseline.cases){if($case -notin $current[0].cases){throw "Retained H test removed: $($baseline.name) $case"}}
    }
    $allowedI=@('src/main/java/com/inigmasgames/hytalerpg/phase00/Phase00Plugin.java',
        'src/main/java/com/inigmasgames/hytalerpg/progress/EncounterGroupCommit.java','src/main/java/com/inigmasgames/hytalerpg/progress/EncounterJournal.java',
        'src/main/java/com/inigmasgames/hytalerpg/progress/EncounterPersistenceTimings.java','src/main/java/com/inigmasgames/hytalerpg/progress/FileEncounterStore.java',
        'src/main/java/com/inigmasgames/hytalerpg/progress/EncounterLog.java','src/main/java/com/inigmasgames/hytalerpg/progress/EncounterJournalV2.java',
        'src/main/java/com/inigmasgames/hytalerpg/progress/EncounterCheckpointBundle.java','src/main/java/com/inigmasgames/hytalerpg/progress/EncounterBarrierArbiter.java',
        'src/main/java/com/inigmasgames/hytalerpg/progress/StorageDurabilityQualification.java')
    $scope=@(& git -C $closureRoot diff --name-only eecd64cc4b7c5345288ffef51083fadc3c8348e4 -- src/main canvas-ui/src)
    $scope+=@(& git -C $closureRoot ls-files --others --exclude-standard -- src/main canvas-ui/src)
    if(@($scope|Where-Object {$_ -notin $allowedI}).Count){throw 'I changed protected gameplay/native/HUD/resources'}
    $crashes=Get-Content -Raw (Join-Path $closureRoot 'build\stage13-hardening\v2-crash-matrix.json')|ConvertFrom-Json
    if($crashes.Count -ne 25 -or @($crashes|Where-Object {$_.exitCode -ne 73 -or $_.restarts -ne 3 -or -not $_.processHaltNotPowerLoss}).Count){throw 'New real-halt matrix incomplete'}
}
if($Cohort -in @('g','h','i','j','k')){
...
if($Cohort -in @('j','k')){
    & "$PSScriptRoot\Test-Stage13HandoffScope.ps1" -Tests $closureTests -Cohort $Cohort
}
...
if($Cohort -in @('c','d','e','f','g','h','i','j','k')){
...
if($Cohort -in @('e','f','g','h','i','j','k')){
    ...
    if($Cohort -in @('f','g','h','i','j','k')){$allowed+=@('src/main/java/com/inigmasgames/hytalerpg/ui/CharacterXpProjectionService.java','src/main/java/com/inigmasgames/hytalerpg/ui/trace/RpgUiTraceService.java')}
...
if($Cohort -in @('j','k')){$changed=@($changed|Where-Object {$_ -ne 'src/main/java/com/inigmasgames/hytalerpg/ui/hud/RpgHudTickSystem.java'})}
...
try{
    ...
    if($Cohort -ne 'a'){
        ...
        if($Cohort -in @('f','g','h','i','j','k')){$required+=@('rpg/runtime/stage-13-stance-cohort-f.json','Server/Item/Items/RPG/Abilities/RPG_Ability_Frenzy.json')}
...
if($Cohort -in @('c','d','e','f','g','h','i','j','k')){
    ...
    if($Cohort -in @('d','e','f','g','h','i','j','k')){$gates+='NATIVE_GUARD_HELD_ITEM_RELEASE_ROUTE_UNVERIFIED'}
    if($Cohort -in @('f','g','h','i','j','k')){$gates+='NATIVE_PER_ACTOR_BASIC_ATTACK_CADENCE_UNVERIFIED'}
...
if($Cohort -in @('f','g','h','i','j','k')){
    ...
    if($Cohort -in @('h','i','j','k')){foreach($name in @('group-crash-matrix.json','group-sparse-load.json')){Copy-Item -LiteralPath (Join-Path $closureRoot "build\stage13-hardening\$name") -Destination (Join-Path $hardening $name) -Force}}
}
...
if($Cohort -in @('j','k')){
    $immediate=Join-Path $closureRoot 'evidence\stage-13\cohort-i\artifacts\HytaleRPG-0.0.25.jar'
    if((Get-FileHash -LiteralPath $immediate).Hash -ne '0082FA775EB2C7C42445194E3E0736515ED21CBCEF77BEC42E04ECA1CFBF3D36'){throw 'Stage I archive changed'}
    $jRollback=Join-Path $closureOut 'rollback\stage13-i';New-Item -ItemType Directory -Force -Path $jRollback|Out-Null
    Copy-Item -LiteralPath $immediate -Destination (Join-Path $jRollback 'HytaleRPG-0.0.25.jar') -Force
    foreach($name in @('v2-crash-matrix.json','v2-group-crash-matrix.json','v2-sparse-load.json','handoff-crash-matrix.json')){Copy-Item -LiteralPath (Join-Path $closureRoot "build\stage13-hardening\$name") -Destination (Join-Path $closureOut "hardening\$name") -Force}
}
$mods=Join-Path $closureRoot "run\stage13-cohort-$Cohort-smoke\mods"
if($Cohort -eq 'k'){
    $previous=Join-Path $closureRoot 'evidence\stage-13\cohort-j\artifacts\HytaleRPG-0.0.25.jar'
    if((Get-FileHash -LiteralPath $previous).Hash -ne '529F7601D69F9DBDA54ABAD87FBF2C674A31C3944E641602AE1BC79225A696AC'){throw 'Immediate J rollback archive mismatch'}
    $destination=Join-Path $closureOut 'rollback\stage13-j';New-Item -ItemType Directory -Force -Path $destination|Out-Null
    Copy-Item -LiteralPath $previous -Destination (Join-Path $destination 'HytaleRPG-0.0.25.jar') -Force
}
if($Cohort -eq 'i'){
    foreach($previous in @('g','h')){
        $previousJar=Join-Path $closureRoot "evidence\stage-13\cohort-$previous\artifacts\HytaleRPG-0.0.25.jar"
        $expectedHash=if($previous -eq 'h'){'8849F88CB9225E847C05580CE1EBE3E2FB6BE0D487C73AA4408526C1ED6381D6'}else{'9B81FAA34D8F41D5C7B43205C52F3E17A44F585D1420C87EB1EEADAB0B7D4FEE'}
        if((Get-FileHash -LiteralPath $previousJar).Hash -ne $expectedHash){throw 'Immediate rollback archive mismatch'}
        $destination=Join-Path $closureOut "rollback\stage13-$previous";New-Item -ItemType Directory -Force $destination|Out-Null
        Copy-Item -LiteralPath $previousJar -Destination (Join-Path $destination 'HytaleRPG-0.0.25.jar')
    }
    foreach($name in @('v2-crash-matrix.json','v2-group-crash-matrix.json','v2-sparse-load.json')){Copy-Item -LiteralPath (Join-Path $closureRoot "build\stage13-hardening\$name") -Destination (Join-Path $closureOut "hardening\$name")}
}
if($Cohort -eq 'h'){
...
[ordered]@{capturedAtUtc=[DateTime]::UtcNow.ToString('o');stage=13;cohort=$Cohort;revision='R032';version='0.0.25';
    status=if($Cohort -in @('j','k')){'LOCAL_CORRECTION_COMPLETE_PENDING_CONNECTED_QA'}else{'IMPLEMENTATION_IN_PROGRESS'};cohortLocalGate='PASS';stageClosure=if($Cohort -in @('j','k')){'BLOCKED_NATIVE_TICK_NOT_MEASURED_AND_NATIVE_INTEGRATION'}elseif($Cohort -in @('f','g','h','i','j','k')){'BLOCKED_DURABLE_CONTRIBUTION_LATENCY_AND_NATIVE_INTEGRATION'}else{'NOT_COMPLETE'};connectedGate='UNVERIFIED';
    tests=$closureCount;failures=0;errors=0;skipped=0;regressionScope=if($Cohort -in @('e','f','g','h','i','j','k')){'COMPLETE_RETAINED_INTERMEDIATE_NOT_STAGE_CLOSURE'}else{'TARGETED_INTERMEDIATE_NOT_STAGE_CLOSURE'};
    ...
    liveDeploymentPerformed=$false;nativeCastingFixed=$false;stage13RollbackDrill=if($Cohort -in @('j','k')){'ACTUAL_STAGE_I_ESCROW_AND_REWARD_COORDINATED_COPY_PASS_NOT_NATIVE_WORLD_PROOF'}elseif($Cohort -in @('f','g','h','i','j','k')){'ACTUAL_ARCHIVED_H_COPIED_CHECKPOINT_PASS_NOT_FINAL_RC'}else{'REQUIRED_AT_CLOSURE_AND_RELEASE_CANDIDATE'};
    explicitRuntimeGates=if($Cohort -in @('c','d','e','f','g','h','i','j','k')){$gates}else{@('BONE_CAGE_NATIVE_ENEMY_ONLY_COLLISION_UNVERIFIED')};
