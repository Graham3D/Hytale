...
$runDirectory = Join-Path $projectRoot "run\stage09-cohort-$Cohort-smoke"
$mods = Join-Path $runDirectory 'mods'
$evidence = Join-Path $projectRoot "evidence\stage-09\cohort-$Cohort"
$stage9Archived = Join-Path $evidence 'artifacts\HytaleRPG-0.0.21.jar'
if ((Test-Path -LiteralPath $stage9Archived) -and
    (Get-FileHash -LiteralPath $stage9Archived).Hash -ne (Get-FileHash -LiteralPath (Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.21.jar')).Hash) {
...
$expectedSupport = switch($Cohort){'a'{3};'b'{8};'c'{12};'d'{16};'e'{16};'f'{16}}
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'build\libs\HytaleRPG-0.0.21.jar') -Destination $resolved
...
try {
    ...
    if (-not $process.Start()) { throw 'Could not start R028 smoke server.' }
    $stdout = $process.StandardOutput.ReadToEndAsync(); $stderr = $process.StandardError.ReadToEndAsync()
    Start-Sleep -Seconds 30
    if (-not $process.HasExited) { $process.StandardInput.WriteLine('stop'); $process.StandardInput.Flush() }
    if (-not $process.WaitForExit(30000)) { $process.Kill($true); throw 'R028 smoke server timeout.' }
...
$summary = [ordered]@{
    ...
    jarSha256 = (Get-FileHash -LiteralPath (Join-Path $resolved 'HytaleRPG-0.0.21.jar')).Hash
    exactlyThreeMods = @(Get-ChildItem -LiteralPath $resolved -Filter '*.jar' -File).Count -eq 3
    rpgDiscovered = [bool]($plain -match 'HytaleRPG-0\.0\.21\.jar')
    rpgSetup = [bool]($plain -match 'HYTALE_RPG_SETUP revision=R028 version=0\.0\.21 hytale=0\.7\.0-pre\.1 stage=09')
    ready = [bool]($plain -match 'RPG_STAGE05_READY revision=R028 .* abilityHud=NATIVE_HYTALE_ONLY')
    packagedRootResolved = [bool]($plain -match 'RPG_NATIVE_BRIDGE_AUDIT revision=R028 root=Root_RPG_Ability_Bridge exists=true operations=2 operation=FirstClickInteraction\+NativeSkillActivationInteraction waitFor=Client operationRemote=true rootRemote=true effectFree=true result=PASS')
    shippedRuneResolved = [bool]($plain -match 'RPG_NATIVE_RUNE_CONTROL_AUDIT revision=R028 item=Rune_Fireball root=Root_Ability_Fireball .* result=PASS connectedProof=false')
...
if (-not ($summary.exactlyThreeMods -and $summary.rpgDiscovered -and $summary.rpgSetup -and $summary.ready -and
    $summary.supportConfigured -and $summary.connectionAssetsResolved -and $summary.areaAssetsResolved -and $summary.packagedRootResolved -and $summary.shippedRuneResolved -and $summary.pluginEnabled -and $summary.managerStarted -and $summary.networkBooted -and $summary.cleanShutdown) -or
    $summary.nativeAbilityAssetsRejected -or $summary.failure) { throw 'R028 smoke gate failed.' }
