...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AM artifact'}
if((Get-FileHash $old).Hash -ne '899C80A31F7165456909EC33DD7BCBC0ECD1EE6843DF0120F2822B037BBAF87D'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.am-staging'
