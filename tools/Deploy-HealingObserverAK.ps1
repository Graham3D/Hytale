...
if($package.jarHashes.'HyARPG.jar' -ne $expected -or (Get-FileHash $candidate).Hash -ne $expected){throw 'Wrong AK artifact'}
if((Get-FileHash $old).Hash -ne '774BD5844BEE9D00C0941A827A4D4F3FF1E87E39C8F1F25D2BCA3AB75200C79E'){throw 'Live RPG build changed: review before replacing'}
...
AssertStopped
$staging=Join-Path $mods 'HyARPG.jar.ak-staging'
