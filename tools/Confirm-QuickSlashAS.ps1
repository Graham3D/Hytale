...
if(-not $package.deployed){throw 'No deployed AS package receipt'}
...
@{revision='R032-AS';activeRpgJarCount=$rpg.Count;installedMods=$jars;changedLiveFiles=$differences;newLiveFiles=$added.Count;verifiedBackupFiles=$inventory.Count;connectedVerified=$false;runtimeEvidence='ISOLATED_EXACT_JAR_SMOKE_ONLY'}|ConvertTo-Json -Depth 6|Set-Content (Join-Path $out 'post-deployment-validation.json') -Encoding utf8
"AS confirmed: one RPG JAR; only mods/HyARPG.jar changed; $($inventory.Count) backup files checked."
