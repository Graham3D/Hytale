...
$base=Join-Path $repo 'evidence\stage-13\cohort-aa\artifacts\HytaleRPG-0.0.25.jar'
$built=Join-Path $repo 'build\libs\HytaleRPG-0.0.25.jar'
$directory=Join-Path $repo 'build\candidate-ab'
...
try{
  foreach($entry in $source.Entries){
    ...
    $allowed=$name -match '^com/inigmasgames/hytalerpg/execution/hytale/(HytaleSkillExecutionSystem|HytaleAreaQueries|NativeBeamTransform|NativeHealingText|HealingTextAccumulator|NativeBlizzardVisuals|NativeProjectileSpawnAuditCommand)(\$[^/]*)?\.class$' -or
      ...
      $name -match '^com/inigmasgames/hytalerpg/ui/hud/(RpgHud|RpgHudCoordinator|CooldownSweep)(\$[^/]*)?\.class$' -or
      $name -in @('com/inigmasgames/hytalerpg/ui/RpgUiProjectionService.class','com/inigmasgames/hytalerpg/ui/model/SkillSlotView.class','com/inigmasgames/hytalerpg/phase00/Phase00Plugin.class','Common/UI/Custom/RpgCooldownSweep.ui','Common/UI/Custom/Assets/RpgHud/CooldownMask.png') -or
...
[ordered]@{baseline=(Get-FileHash $base).Hash;jarSha256=(Get-FileHash $candidate).Hash;changedEntries=@($changed);otherAAEntriesByteIdentical=$true;deployed=$false} |
