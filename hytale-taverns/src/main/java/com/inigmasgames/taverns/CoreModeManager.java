...
final class CoreModeManager {
    ...
    private static UpdatePlayerInventory createInventorySnapshot(
        ...
        InventoryComponent.AbilitySlots abilitySlots =
                store.getComponent(ref, InventoryComponent.AbilitySlots.getComponentType());
        InventoryComponent.RuneBag runeBag =
                store.getComponent(ref, InventoryComponent.RuneBag.getComponentType());
        return new UpdatePlayerInventory(
                section(storage), section(armor), hotbar, section(utility), section(tools),
                section(backpack), section(abilitySlots), section(runeBag));
    }
