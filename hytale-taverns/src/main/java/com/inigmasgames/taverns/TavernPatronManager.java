...
final class TavernPatronManager {
    ...
    private boolean tickGreeting(
        ...
        if (session.greetingPhase == GreetingPhase.WAITING) {
            ...
            if (npc.getRole() != null) {
                npc.getRole().setMarkedTarget(
                        session.patronRef, commandBuffer, GREETING_TARGET_SLOT, player);
                MarkedEntitySupport marked = commandBuffer.getComponent(
                        session.patronRef, MarkedEntitySupport.getComponentType());
                Ref<EntityStore> boundTarget = marked == null
                        ? null
                        : marked.getMarkedEntityRef(GREETING_TARGET_SLOT);
    ...
    private static void clearGreetingTarget(
        ...
        if (npc != null && npc.getRole() != null) {
            npc.getRole().setMarkedTarget(
                    session.patronRef, commandBuffer, GREETING_TARGET_SLOT, null);
    ...
    private static void applyGreetingHeadTracking(
        ...
        npc.getRole().setMarkedTarget(
                session.patronRef, commandBuffer, GREETING_TARGET_SLOT, session.greetingTarget);
        MarkedEntitySupport marked = commandBuffer.getComponent(
                session.patronRef, MarkedEntitySupport.getComponentType());
        Ref<EntityStore> boundTarget = marked == null
                ? null
