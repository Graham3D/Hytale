...
final class CoreModePermissionProvider implements PermissionProvider {
    ...
    @Override public Set<String> getUserPermissions(UUID uuid) {
        return activePlayers.contains(uuid)
                ? Set.of(HytalePermissions.EDITOR_SELECTION_USE.getId())
                : Collections.emptySet();
    }

    @Override public Set<UUID> getUsersWithPermission(String permission) {
        return HytalePermissions.EDITOR_SELECTION_USE.getId().equals(permission)
                ? Set.copyOf(activePlayers)
                : Collections.emptySet();
    }
