...
class Stage12PersistentEncounterRuntimeTest {
    ...
    private FileEncounterStore store(){return new FileEncounterStore(directory);}
    private PersistentEncounterRuntime runtime(){return new PersistentEncounterRuntime(store(),(player,reward)->awards.add(reward));}
    ...
    @Test void conversionRestorationCannotRestoreRewardEligibility(){var r=begin();hit(r,101);r.disqualify(world,enemy);assertFalse(r.contains(world,enemy));assertFalse(runtime().attach(world,enemy,"Wolf_Black",Optional.empty()));assertTrue(r.death(world,enemy,Vec3.ZERO,102,participants()).isEmpty());}
    ...
    @Test void partialStoreFailureFreezesProgressionAndRetainsNativeCombatInputs() {
        var failure=new FileEncounterStore(directory,boundary->{if(boundary==FileEncounterStore.Boundary.AFTER_CONTEXT)throw new IllegalStateException("DISK_UNCERTAIN");});
