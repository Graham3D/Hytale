...
class Stage11FoundationTest {
    ...
    @Test void projectileRangeChangesOnlyTravelWhileAuthoredSafetyLifetimeRemainsBounded(){var authored=profiles.require("fire_bolt").projectile();var p=effective("fire_bolt","long_reach").projectile();assertEquals(32.5,p.maxDistance());assertEquals(authored.radius(),p.radius());assertEquals(authored.speed(),p.speed());assertEquals(1,p.maximumLifetimeSeconds(),1e-9);assertEquals(authored.statusSeconds(),p.statusSeconds());}
    @Test void reachNotAppliedTwiceWhenResolvingRepeatedly(){var authored=profiles.require("fire_bolt");var p=plan("fire_bolt","long_reach");var a=resolver.resolve(authored,p);for(int i=0;i<100;i++)assertSame(a,resolver.resolve(authored,p));assertEquals(26,authored.projectile().maxDistance());assertEquals(1,resolver.cachedProfiles());}
