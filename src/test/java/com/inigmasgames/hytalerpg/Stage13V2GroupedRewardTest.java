...
class Stage13V2GroupedRewardTest {
    private static FileEncounterStore open(Path directory,java.util.function.Consumer<FileEncounterStore.Boundary> death,java.util.function.Consumer<FileEncounterStore.DurabilityBoundary> fault,java.util.function.Consumer<FileEncounterStore.GroupBoundary> group){return FileEncounterStore.durableV2(directory,death,fault,group,true);}
    ...
    void groupedContributionsThenInterruptedDeathDeliveryPayEveryParticipantExactlyOnce(FileEncounterStore.Boundary boundary)throws Exception{
        ...
        for(int restart=0;restart<3;restart++)try(var store=FileEncounterStore.durableV2(directory.resolve("encounters"))){
