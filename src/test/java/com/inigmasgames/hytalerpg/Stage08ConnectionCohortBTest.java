...
class Stage08ConnectionCohortBTest {
    @Test void exactFiveProfilesCompleteStage08WithoutExpandingCatalog() {
        var p=Stage04SkillProfiles.loadCanonical(Stage01BTestSupport.bundle().catalog());assertEquals(35,p.all().values().stream().filter(profile->profile.support()==null&&profile.summon()==null&&profile.summonAction()==null&&profile.conversion()==null&&profile.cage()==null).count());
