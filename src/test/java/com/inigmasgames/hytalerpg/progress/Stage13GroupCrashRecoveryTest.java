...
class Stage13GroupCrashRecoveryTest {
    ...
    void haltWithoutShutdownRecoversOnlyTheReplayablePrefix(FileEncounterStore.GroupBoundary boundary)throws Exception{
        var cp=new ArrayList<String>();for(Class<?> type:List.of(EncounterGroupCrashProcess.class,FileEncounterStore.class,com.google.gson.Gson.class))cp.add(Path.of(type.getProtectionDomain().getCodeSource().getLocation().toURI()).toString());
        cp.add(Path.of("build/resources/main").toAbsolutePath().toString());Path log=directory.resolve("child.log");
        var child=new ProcessBuilder(Path.of(System.getProperty("java.home"),"bin/java.exe").toString(),"-cp",String.join(System.getProperty("path.separator"),cp),EncounterGroupCrashProcess.class.getName(),directory.toString(),boundary.name()).redirectErrorStream(true).redirectOutput(log.toFile()).start();
        ...
        for(int restart=0;restart<3;restart++)try(var store=new FileEncounterStore(directory)){
    ...
    @AfterAll static void evidence()throws Exception{Path file=Path.of("build/stage13-hardening/group-crash-matrix.json");Files.createDirectories(file.getParent());Files.writeString(file,new com.google.gson.GsonBuilder().setPrettyPrinting().create().toJson(evidence));}
}
