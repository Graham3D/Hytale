...
class Stage12ProgressionMathTest {
    ...
    @Test void biomeBandsDoNotPretendToBeAuditedGameIds(){var p=ProgressionProfiles.load();assertEquals(5,p.biomeBands().size());for(int level=1;level<=99;level++){final int n=level;assertEquals(1,p.biomeBands().stream().filter(b->b.contains(n)).count());}assertTrue(p.nativeBiome("made_up_native_biome").isEmpty());assertTrue(p.biomeBands().stream().allMatch(b->b.verifiedNativeBiomeIds().isEmpty()));}
