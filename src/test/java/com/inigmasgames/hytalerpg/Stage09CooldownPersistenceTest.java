...
class Stage09CooldownPersistenceTest {
    ...
    @Test void auraRateExpiresAtDisconnectAndIsNeverSerialized(){
        ...
        assertEquals(9.77,h.cd.remaining(h.actor,"skill"),1e-9);assertEquals(Set.of("remainingWork","baseRecovery","dynamicOwnerRate","queued"),new com.google.gson.Gson().toJsonTree(h.saved.get("skill")).getAsJsonObject().keySet());
    }
    ...
    @Test void savedCooldownMapRejectsUnboundedOrMalformedState(){
        ...
        var values=new HashMap<String,SavedCooldown>();for(int i=0;i<=com.inigmasgames.hytalerpg.content.RpgCatalog.EXPECTED_SKILLS;i++)values.put("s"+i,new SavedCooldown(1,0));
