...
class Stage05ProjectileTest {
    @Test void sixProjectileProfilesPreserveCanonicalCohortAndExcludeSnipe() {
        ...
        assertEquals(12, profiles.all().values().stream().filter(profile -> profile.area() == null && profile.connection() == null && profile.support() == null && profile.summon() == null && profile.summonAction() == null && profile.conversion() == null).count());
    ...
    private record Harness(UUID actor, Stage01BTestSupport.Bundle bundle, RpgCombatKernel kernel,
        ...
        SkillExecutionResult execute() {
            return service.request(new SkillExecutionRequest(actor, SkillSlot.SKILL01, "Ability2", 51,
