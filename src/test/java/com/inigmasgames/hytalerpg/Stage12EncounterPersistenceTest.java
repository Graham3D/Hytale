...
class Stage12EncounterPersistenceTest {
    ...
    private FileEncounterStore failing(FileEncounterStore.Boundary at){var once=new AtomicBoolean();return new FileEncounterStore(directory.resolve("encounters"),boundary->{if(boundary==at&&once.compareAndSet(false,true))throw new IllegalStateException("PROCESS_INTERRUPTED");});}
    ...
    @Test void initialContextSurvivesRestartWithOriginalBiomeAndIdentity(){var first=store().create(spawn(enemy));assertEquals(first,store().load(world,enemy).orElseThrow());assertEquals(5,first.spawn().level());assertEquals(-1,first.firstCombat());}
    @Test void unknownLoadDoesNotClassifyOrWriteContext(){assertTrue(store().load(world,enemy).isEmpty());assertFalse(Files.exists(file("contexts",enemy)));}
    ...
    @Test void conversionTombstoneSurvivesRestoreAndCannotBeCleared(){var ledger=ledger();var clean=ledger.snapshot(world,enemy);store().disqualify(world,enemy);assertTrue(store().load(world,enemy).orElseThrow().disqualified());assertThrows(IllegalStateException.class,()->store().save(clean));assertThrows(IllegalStateException.class,()->store().create(spawn(enemy)));}
    @Test void conversionBeforeCapturePermanentlyPreventsClassification(){store().disqualify(world,enemy);store().disqualify(world,enemy);assertThrows(IllegalStateException.class,()->store().create(spawn(enemy)));assertTrue(store().load(world,enemy).isEmpty());}
    ...
    @Test void cannotWriteCreditsOrRecreateAfterDeath(){var plan=plan();var snapshot=store().load(world,enemy).orElseThrow();store().freeze(plan);assertThrows(IllegalStateException.class,()->store().save(snapshot));assertThrows(IllegalStateException.class,()->store().create(spawn(enemy)));}
    ...
    @Test void interruptedContextCreationRemainsOriginalOnRestart(){assertThrows(IllegalStateException.class,()->failing(FileEncounterStore.Boundary.AFTER_CONTEXT).create(spawn(enemy)));assertEquals(spawn(enemy),store().load(world,enemy).orElseThrow().spawn());}
    ...
    @Test void mismatchedFileIdentityFailsClosed()throws Exception{store().create(spawn(enemy));var other=UUID.randomUUID();Files.createDirectories(file("contexts",other).getParent());Files.copy(file("contexts",enemy),file("contexts",other));assertThrows(IllegalStateException.class,()->store().load(world,other));}
    @Test void corruptSnapshotIsRetainedNotReset()throws Exception{store().create(spawn(enemy));Files.writeString(file("contexts",enemy),"{broken");assertThrows(IllegalStateException.class,()->store().load(world,enemy));assertEquals("{broken",Files.readString(file("contexts",enemy)));}
    ...
    @Test void invalidSnapshotShapeRejectedEvenWithUpdatedChecksum()throws Exception{store().create(spawn(enemy));Path path=file("contexts",enemy);var json=JsonParser.parseString(Files.readString(path)).getAsJsonObject();json.getAsJsonObject("payload").remove("disqualified");json.addProperty("checksum",RewardIntent.digest(new Gson().toJson(json.get("payload"))));Files.writeString(path,json.toString());assertThrows(IllegalStateException.class,()->store().load(world,enemy));}
    @Test void oversizedSnapshotRejectedBeforeParsing()throws Exception{store().create(spawn(enemy));Files.writeString(file("contexts",enemy)," ".repeat(FileEncounterStore.MAX_FILE_BYTES+1));assertThrows(IllegalStateException.class,()->store().load(world,enemy));}
    ...
    @Test void concurrentWriterCannotEnterStore()throws Exception{store().pendingCount();try(var channel=FileChannel.open(directory.resolve("encounters/writer.lock"),StandardOpenOption.WRITE);var lock=channel.lock()){assertThrows(IllegalStateException.class,()->store().create(spawn(enemy)));}}
    ...
    @Test void restoreNeverOverwritesLiveEncounter(){var ledger=ledger();assertThrows(IllegalStateException.class,()->ledger.restore(store().load(world,enemy).orElseThrow()));assertEquals(1,ledger.size());}
    @Test void restoredIndicesCleanUpOnUnload(){var plan=plan();var ledger=new EncounterContributions();ledger.restore(store().load(world,enemy).orElseThrow());ledger.unload(world);assertEquals(0,ledger.size());assertEquals(0,ledger.heal(world,b,a,5,true,104));assertEquals(2,plan.shares().size());}
