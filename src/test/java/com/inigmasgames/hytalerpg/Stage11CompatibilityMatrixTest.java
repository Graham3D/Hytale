...
class Stage11CompatibilityMatrixTest {
    ...
    @Test void all6097SkillPassiveCellsHaveExplicitVerdicts()throws Exception{
        ...
        assertEquals(6432,rows.size());assertEquals(67,eligible.size(),"Every primitive needs an implemented positive profile");
    ...
    @Test void all2211PairsClassifiedAgainstAll91Skills()throws Exception{
        ...
        for(int a=0;a<passives.size();a++)for(int b=a+1;b<passives.size();b++){
            ...
            assertEquals(96,accepted.size()+missing.size()+gated.size()+capabilities.size()+rejected.values().stream().mapToInt(Integer::intValue).sum());
        }
