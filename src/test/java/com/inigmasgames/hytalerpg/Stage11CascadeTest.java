...
class Stage11CascadeTest {
    ...
    static class H extends Stage09SupportRuntimeTest.Harness implements AreaWorldPort {
        ...
        public Optional<Vec3> sweepShard(Vec3 from,Vec3 to,double radius){return ground&&from.y()>0&&to.y()<=radius?Optional.of(new Vec3(to.x(),0,to.z())):Optional.empty();}
    ...
    @Test void finiteBudgetCapsBlizzardImpactsWithoutDroppingEitherChildField(){var h=new H("blizzard","cascade","rapid_pulse","lingering");h.start();assertEquals(3,h.areas.size());for(int i=1;i<=100;i++)h.step(i*.05);assertEquals(48,h.context.effects().spawned());assertEquals(2,h.context.effects().triggered());assertTrue(h.reasons.contains("ROOT_SPAWN_EFFECT_BUDGET"));assertEquals(0,h.areas.size());}
