...
public final class HealingProbeCommand extends AbstractPlayerCommand {
    ...
    @Override protected void execute(CommandContext context,Store<EntityStore> store,Ref<EntityStore> ref,PlayerRef player,World world){
        try{
            ...
            context.sendMessage(Message.raw("R032-AP requested. Wait for STARTED/ARMED or FAILED. channel none observes real Healing Beam casts for 30s without changing them. Old visual controls still run for 10s; wait 12s between them."));
