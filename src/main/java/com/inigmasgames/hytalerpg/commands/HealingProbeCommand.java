...
public final class HealingProbeCommand extends AbstractPlayerCommand {
    ...
    public HealingProbeCommand(HealingPresentationProbe probe){
        ...
        target=withRequiredArg("target","native (temporary native NPC) or current runtime entity UUID; ignored by stop",ArgTypes.STRING);
    }
    @Override protected void execute(CommandContext context,Store<EntityStore> store,Ref<EntityStore> ref,PlayerRef player,World world){
        try{
            ...
            context.sendMessage(Message.raw("R032-AI visual probe requested for 10s. No healing/cost. Native target spawns 6m toward world -Z. Record what is visible; server receipts do not prove rendering. Wait 12s before next control."));
