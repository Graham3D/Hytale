...
public final class HytaleSkillExecutionSystem extends EntityTickingSystem<EntityStore>
    ...
    public double activeSkillRemaining(UUID owner,String skill){
        ...
        if(skill.equals("lightning_coil"))return spires.inspectOwnerAll(owner,now).stream().mapToDouble(value->value.phase()==com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.Phase.EMERGING
                ?value.readyRemaining()+com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.EMERGENCE_SECONDS*(1-value.emergenceProgress()):value.readyRemaining()).max().orElse(0d);
    ...
    private boolean onFriendlyConstructMelee(UUID actor,Ref<EntityStore> target,String hitIdentity,Store<EntityStore> store,
        ...
        if(result.code().equals("CHARGED")||result.code().equals("DISCHARGE")){
            ...
            if(context!=null)emit(context,RpgTraceEventType.AREA_PRESENTATION,Map.of("phase","LIGHTNING_SPIRE_FRIENDLY_HIT",
                    "contributor",actor,"shockModel","Hywind_Lightning_Spire_Shock_Frame_0","shockFrames",8,"frameMilliseconds",100,"gaugePercent",
    ...
    private void advanceLightningSpire(UUID owner,double now,double delta,Port port){
        spireVisuals.tickHitFlashes(now,port.buffer);
        for(var view:spires.inspectOwnerAll(owner,now)){
            var context=spireContexts.get(view.instance());
            if(context==null){spires.cancel(view.instance(),com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.EndReason.OWNER_CLEANUP);spireVisuals.end(view.instance(),false,port.buffer);continue;}
            ...
            if(view.phase()==com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.Phase.READY
                ...
                emit(context,RpgTraceEventType.AREA_PRESENTATION,Map.of("phase","LIGHTNING_SPIRE_READY","attackable",true,
                        "readySeconds",view.readyRemaining(),"maximumHealth",view.maximumHealth(),"connectedProof",false));
            ...
            if(nativeHealth.destroyed()){
                ...
                executions.terminate(context,"LIGHTNING_SPIRE_DESTROYED");continue;
            }
            var expired=spires.expire(view.instance(),now);
            if(expired.isPresent()){
                ...
                executions.terminate(context,"LIGHTNING_SPIRE_READY_EXPIRED");continue;
            }
            for(var wave:spires.drainWaves(view.instance())){
    ...
    private void cancel(UUID actor, String reason, CommandBuffer<EntityStore> buffer) {
        ...
        for(var ended:spires.cancel(actor,com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.EndReason.OWNER_CLEANUP)){
            spireContexts.remove(ended.instance());spireVisuals.end(ended.instance(),false,buffer);
        }
        ...
        for(var connection:connections.cancel(actor,false)) {
            inputs.stopHeld(connection.request());
    ...
    private final class Port implements SkillExecutionPort {
        ...
        @Override public SkillExecutionResult commandExisting(Stage04SkillProfile profile,com.inigmasgames.hytalerpg.domain.CompiledSkillPlan plan,SkillExecutionRequest request){
            if(!profile.skillId().equals("bomb_toss"))return null;
            var ground=projectileFuses.values().stream().filter(value->value.carrier.actorId.equals(playerRef.getUuid())
                    &&value.carrier.context.profile().skillId().equals("bomb_toss")).findFirst().orElse(null);
            if(ground==null)return null;var carrier=ground.carrier;String id=carrier.instance.plan().projectileInstanceId();
            var point=carrier.detonation.contact(ground.point,true,System.nanoTime()/1e9).orElse(null);if(point==null)return null;
            if(!projectileFuses.remove(id,ground))return SkillExecutionResult.rejected("COMMAND_INSTANCE_RACE");
            try{applyAuthoredExplosion(carrier.context,this,point,id+"/command-existing",carrier.instance.plan().generation()==0);
                return SkillExecutionResult.committed("EXISTING_INSTANCE_COMMANDED",0,0);
            }finally{projectileService.onForwardTermination(carrier.instance,"COMMAND_EXISTING",point);finishProjectileContext(carrier.context,"COMMAND_EXISTING");}
        }
        ...
        @Override public Validation familyPrerequisites(Stage04SkillProfile profile,
            ...
            if (profile.area() != null) {
                String admitted = profile.skillId().equals("lightning_coil")
                        ?"PASS":areas.admission(playerRef.getUuid(), profile.skillId(), profile.area().trap());
                if (!admitted.equals("PASS")) return Validation.reject(admitted);
                Vec3 feet = vec(store.getComponent(actor, TransformComponent.getComponentType()).getPosition());
        ...
        private ConnectionWorldPort connectionWorld() {
            return new ConnectionWorldPort() {
                ...
                public boolean held(SkillExecutionContext context){return inputs.held(context.request());}
                public java.util.Optional<Target> resolveFriendly(String id){
                    ...
                    return java.util.Optional.of(new Target(id,new AreaGeometry.Bounds(vec(b.min).add(position),vec(b.max).add(position))));
                }
                public Query queryFriendly(ConnectionShape shape,int cap){
                    var targets=new ArrayList<Target>();
                    boolean overflow=store.forEachChunk(com.hypixel.hytale.component.query.Query.and(UUIDComponent.getComponentType(),EntityStatMap.getComponentType(),BoundingBox.getComponentType(),TransformComponent.getComponentType()),(chunk,ignoredBuffer)->{
                        for(int i=0;i<chunk.size();i++){
                            var id=chunk.getComponent(i,UUIDComponent.getComponentType()).getUuid().toString();
                            var value=resolveFriendly(id).orElse(null);
                            if(value!=null&&shape.intersects(value.bounds())){if(targets.size()>=cap)return true;targets.add(value);}
                        }
                        return false;
                    });
        ...
        @Override public SkillExecutionResult executeArea(SkillExecutionContext context) {
            ...
            if(context.profile().skillId().equals("lightning_coil")){
                ...
                }catch(RuntimeException failure){spires.cancel(context.skillInstanceId(),com.inigmasgames.hytalerpg.execution.lightning.LightningSpireRuntime.EndReason.OWNER_CLEANUP);spireVisuals.end(context.skillInstanceId(),false,buffer);throw failure;}
            }
        ...
        private AreaWorldPort areaWorld() {
            return new AreaWorldPort() {
                ...
                @Override public void trace(SkillExecutionContext context, String event, Map<String, ?> details) {
                    ...
                    if(event.equals("AREA_TERMINATED"))executions.terminate(context,String.valueOf(details.get("reason")));
                }
            };
        }
        ...
        @Override public SkillExecutionResult executeProjectile(SkillExecutionContext context) {
            ...
            try {
                ...
                if(isGroundSpark(context)) {
                    direction=direction.horizontalNormalized();
                    Vec3 feet=vec(store.getComponent(actor,TransformComponent.getComponentType()).getPosition());
                    Vec3 release=feet.add(direction.multiply(.55));
                    Vec3 ground=HytaleAreaQueries.ground(store,release.add(new Vec3(0,1.6,0)),new Vec3(0,-1,0),3)
                            .orElseThrow(()->new IllegalStateException("SPARK_GROUND_ORIGIN_UNAVAILABLE"));
                    if(Math.abs(ground.y()-feet.y())>1.1)throw new IllegalStateException("SPARK_GROUND_ORIGIN_OUT_OF_TOLERANCE");
                    origin=ground.add(new Vec3(0,.49,0));
                }
    ...
    private void onProjectileImpact(ProjectileCarrier expected, Ref<EntityStore> projectileRef,
        ...
        if(!isGroundSpark(carrier.context)&&expireContinuationClock(carrier,buffer))return;
        ...
        if (hitEntity != null && hitEntity.isValid()) {
            if(hasContinuations(carrier.context) && !isGroundSpark(carrier.context)
                    && !HytaleAreaQueries.clear(store,carrier.instance.flight().lastPosition(),vec(position))) {
    ...
    private void advanceProjectiles(UUID actorId, float deltaSeconds, Store<EntityStore> store,
        ...
        for (ProjectileCarrier carrier : owned) {
            ...
            if(isGroundSpark(carrier.context)) {
                advanceGroundSpark(carrier,deltaSeconds,store,buffer);
                continue;
            }
    ...
    private static boolean hasContinuations(SkillExecutionContext context) {
        var modifiers=context.compiledPlan().projectileModifiers();
        return isGroundSpark(context)||modifiers.pierce()+modifiers.fork()+modifiers.chain()+modifiers.returning()+modifiers.ricochet()>0
    ...
    private static boolean isGroundSpark(SkillExecutionContext context) {
        return context!=null&&context.profile()!=null&&context.profile().skillId().equals("charged_bolt");
    }
    ...
    private void advanceGroundSpark(ProjectileCarrier carrier,float deltaSeconds,Store<EntityStore> store,
            CommandBuffer<EntityStore> buffer) {
        try {
            if(!Float.isFinite(deltaSeconds)||deltaSeconds<0)throw new IllegalStateException("SPARK_INVALID_TICK_DELTA");
            var instance=carrier.instance;var steering=carrier.sparkSteering;
            if(steering==null)throw new IllegalStateException("SPARK_STEERING_MISSING");
            steering.synchronize(instance.direction(),instance.motionRevision());
            Vec3 current=instance.flight().lastPosition();
            double remaining=Math.min(instance.remainingDistance(),instance.plan().velocity().length()*deltaSeconds);
            int steps=0;
            while(remaining>1e-8) {
                ...
                moveGroundSparkCarrier(carrier,current,buffer);
                if(observation.expired()) {finishGroundSparkTravel(carrier,current,buffer);return;}
            }
            var elapsed=instance.observe(deltaSeconds,current);
            if(elapsed.expired())finishGroundSparkTravel(carrier,current,buffer);
        } catch(RuntimeException error) {
    ...
    private void finishGroundSparkTravel(ProjectileCarrier carrier,Vec3 point,CommandBuffer<EntityStore> buffer) {
        String reason=carrier.instance.remainingDistance()<=1e-6?"MAX_RANGE":"MAX_LIFETIME";
        var decision=continuations.forwardEnd(carrier.instance,point,casterPoint(carrier,buffer.getStore()),reason);
        if(decision.action()==ProjectileContinuation.Action.TERMINATE)finishProjectileTerminal(carrier,reason,point,buffer);
        else applyContinuation(carrier,decision,point,buffer);
    }
    private void moveGroundSparkCarrier(ProjectileCarrier carrier,Vec3 point,CommandBuffer<EntityStore> buffer) {
        ...
        if(physics==null||transform==null||velocity==null)throw new IllegalStateException("SPARK_NATIVE_COMPONENT_MISSING");
        physics.getPosition().set(vector(point));physics.getVelocity().set(0,0,0);physics.setState(StandardPhysicsProvider.STATE.ACTIVE);
    ...
    private boolean processSweptContacts(ProjectileCarrier carrier,Vec3 destination,CommandBuffer<EntityStore> buffer) {
        ...
        for(var contact:contacts) {
            ...
            if(!isGroundSpark(carrier.context)&&!HytaleAreaQueries.clear(buffer.getStore(),from,point))break;
            ...
            buffer.run(store->{
                ...
                if(bound.projectile.isValid())
                    emitProjectile(bound,RpgTraceEventType.PROJECTILE_SPAWNED,Map.of("configId",plan.configId(),"speed",plan.velocity().length(),
                            ...
                            "groundCrawler",isGroundSpark(bound.context),"verticalVelocity",plan.velocity().y()));
            });
            spawnStage=ProjectileSpawnDiagnostics.Stage.PRESENTATION;
            if(!isGroundSpark(context)&&context.effects().projectileVisuals().cast(context.skillInstanceId()+"/"+context.barrageBatch()))projectileVisual(carrier,buffer.getStore(),()->vfx.presentContact(buffer.getStore().getExternalData().getWorld(),plan.origin(),
    ...
    private void resumeCarrier(ProjectileCarrier carrier,Vec3 point,CommandBuffer<EntityStore> buffer,boolean resetHitPhase) {
        ...
        physics.getPosition().set(vector(point));
        if(isGroundSpark(carrier.context))physics.getVelocity().set(0,0,0);
        else physics.getVelocity().set(vector(carrier.instance.direction().multiply(carrier.instance.plan().velocity().length())));
    ...
    private void emitAuthoredProjectileCast(SkillExecutionContext context,List<ProjectileExecutionPlan> plans){
        ...
        Vec3 castDirection=isGroundSpark(context)?first.velocity().normalized():context.target().direction();
        details.put("directionX",castDirection.x());details.put("directionY",castDirection.y());details.put("directionZ",castDirection.z());
