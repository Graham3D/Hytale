...
public final class SummonRegistry {
    ...
    public static final class Lease {
        ...
        private int attacks;
        private Lease(SkillExecutionContext context,double now) {
            this.context=context;expires=now+context.profile().summon().lifetime();
            nextAttack=now+context.profile().summon().attackInterval();
        }
    ...
    public synchronized List<Lease> reserve(SkillExecutionContext context,double now) {
        ...
        for(int i=0;i<count;i++){var lease=new Lease(context,now);leases.put(lease.token,lease);result.add(lease);}
    ...
    public synchronized int claimAttack(UUID token,double now) {
        ...
        lease.nextAttack=now+lease.context.profile().summon().attackInterval();return ++lease.attacks;
    }
