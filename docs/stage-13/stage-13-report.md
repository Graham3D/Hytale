...
**Latest correction T:** [Snipe hold/release report](stage-13-snipe-release-report.md) implements the owner's revised full-draw-on-hold / fire-on-release contract. Snipe alone uses native Charging plus one server RPG release callback; charged physics are 85 m/s, gravity 25, radius 0.075, with an explicitly authored 48 m cap. Damage/payment/ammunition are unchanged; the old native-maximum requirement is superseded by owner instruction, not declared proven. All 2,164 retained tests passed. See the correction report for final packaging/deployment receipts. Connected hold/release, pose and flight QA remain required; Stage 13 is not PASS.

**Latest update, September 9 local / September 10 UTC, Correction S:** [Owner-managed icon report](stage-13-owner-icons-report.md) adds a complete 87-skill/66-passive filename map, optional packaged artwork lookup, passive previews, and a validated one-click PNG importer with backups/undo. R032-S includes the owner's new Whirlwind icon and is deployed; R is retained. All **2,157 tests**, exact-JAR native smoke, three-mod archive and rollback checks pass; live saves/data are unchanged. [Owner guide](../../art/ADDING-ICONS.md) explains the independent workflow. **Connected icon/UI QA remains required.** No gameplay or persistence changes; known encounter/post-lethal issues remain unresolved and Stage 13 is not PASS.

**Latest update, September 9 local / September 10 UTC, Correction R:** [Skill icons/search report](stage-13-skill-icons-search-report.md) fixes the Skill Tree dynamic query codec key, adds search labels on both tabs and integrates unchanged owner Fire Bolt/Quick Slash overlays in native skill slots and the Skill Tree. The native asset loader rejected the first icon namespace; corrected `Icons/Items/RPG/` assets passed exact-JAR smoke before deployment. All **2,145 tests**, three-mod archive, byte-differential and rollback checks passed. R032-R is deployed, Q retained, live data unchanged. **Connected UI QA is still required.** Q's post-lethal/encounter uncertainty remains unresolved; no gameplay or persistence changes. Stage 13 is not PASS.

**Latest update, September 9 local / September 10 UTC, Correction Q:** [Quick Slash log/speed report](stage-13-quick-slash-review-report.md) documents 13 connected P Quick Slash casts: 10 zero-candidate queries and three damaging roots with 157 authoritative Health lost. Post-lethal execution errors and `NATIVE_CONTRIBUTION_INSPECT` encounter uncertainty are explicitly **unresolved**. Q applies the requested +200% (3× base) animation speed and bounded failure code-location telemetry; no speculative damage or persistence changes. All 2,137 tests, exact-JAR native smoke, archive and rollback checks passed; Q deployed, live data unchanged, P retained. Connected Q and progression acceptance remain unverified; Stage 13 is not PASS.

**Latest update, 2026-09-09, Correction P:** [player feedback correction report](stage-13-player-feedback-correction-report.md) records the connected same-tick projectile ownership race, native resting-expiry proof, two full-power Quick Slash swings at 1.5× native animation speed, the owner-authorized complete-pair Multistrike exception, native Chill debuff icons and restored R032-P badge. All **2,135 tests** and exact-JAR three-mod/native smoke, archive and rollback checks passed. P is deployed; O is retained for rollback and live data is unchanged. **Connected P animation/flight/impact/HUD QA and formal native performance remain UNVERIFIED. Stage 13 is not PASS.** Earlier update paragraphs are historical.

**Latest update, 2026-09-09, Correction O:** [native Fire Bolt spawn correction](stage-13-native-projectile-spawn-correction-report.md) fixes the reproduced native empty-EnumMap exception and premature reads of a queued entity Ref. The exact deployed JAR reached `PROJECTILE_SPAWNED` with real native components in an isolated three-mod server; pending rollback also passed. All 2,128 retained tests passed. N is retained for rollback; saves, N weapon coverage and NORMAL trace aggregation are preserved. **Connected O rendering/flight/impact and formal native performance remain UNVERIFIED; Stage 13 is not PASS.** The N paragraph below is historical; the owner subsequently confirmed N Quick Slash and Fire Bolt validation, localizing O to native spawn construction.

**Latest update, 2026-09-09:** [cohort N power/trace correction](stage-13-power-trace-correction-report.md) addresses M's connected Flame Longsword/Mithril Staff power failures, expands the pinned production audit to 197 weapons, and adds NORMAL/DETAILED/PERFORMANCE trace routing. All 2,119 tests pass; exact three-mod smoke, packaging and rollback checks pass. N is deployed, with saves preserved and M retained for rollback. **N connected casting and formal native performance remain UNVERIFIED; Stage 13 is not PASS.** Mithril Staff is now an explicit authored-power positive test; the obsolete unsupported-Mithril instruction in M is superseded. [M](stage-13-casting-correction-report.md) and [L](stage-13-startup-hotfix-report.md) remain historical. The [consolidated history/QA packet](../review/README.md) retains its L checkpoint; use N's report for this correction's current checklist. Earlier cohort/no-deployment statements below retain their historical scope.
