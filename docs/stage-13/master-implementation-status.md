...
**2026-09-12 latest:** [R032-AH Healing presentation / Mana report](stage-13-presentation-ah-report.md). **DEPLOYED** `HyARPG.jar` repairs AG's client-logged missing texture, moves Staff_Bronze from permanent item particles to finite active-channel effects, and registers RPG skill execution in native stat-modification ordering. **2,246 retained tests pass**, plus installed-byte native lifecycle / one-charge Mana update-queue checks. **Connected rendering and Mana-bar synchronization are not yet verified.** Existing gameplay, persistence and open release gates are unchanged. Earlier entries below are historical.
