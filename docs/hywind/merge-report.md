...
R065 is implemented, packaged, and deployed from implementation commit `d220c2b80e26a0f69c1eb239d336a7fb8ece4450`. It replaces Spark's point-sized particle with a horizontal one-block plane, plays all four 80x80 `chargedbolt.png` frames at 100 ms per frame, and keeps the plane attached to the custom ground-resolved carrier. Spark now travels at 12 m/s over a 10 m actual-path budget; the matching native projectile launch and terminal velocities are synchronized with that runtime contract. Combat balance and collision/passive authority are unchanged.

The installed `Hywind.jar` SHA-256 is `1D577A47E9475E4EA392AA837A82909FB915978724219CB8C09FC56C036C94C0`. The verified full-save rollback is `C:\Users\Zemio\OneDrive\Documents\GitHub\Hytale-rollback\hywind-deploy-20260920T190709Z`. The full retained suite, package audit, isolated smoke, deployment dry run, and two deployed startup/restart cycles pass. Connected-client presentation and travel-feel confirmation remains pending; see `r065-spark-presentation-and-travel.md`.

## R066 Spark world-quad visual hotfix

R066 is implemented, packaged, and deployed from implementation commit `ac7a77d25a1a9db4a80134d4824573e2ec4f3278`. It fixes the actual presentation failure instead of retuning the failed particle: the former projectile model's `0.06` parent scale collapsed the intended surface while a separate yellow light remained visible as a point. Spark now uses a native fullbright model quad with a fixed `+Y` normal, an exact one-block footprint, and a four-frame 100 ms UV animation sourced from the canonical transparent blue/white sheet. The particle emitter, Fireball carrier mesh, inherited tiny scale, and yellow light are gone. Spark gameplay authority is unchanged.

The installed `Hywind.jar` SHA-256 is `5541596CF3EDA107658EAFB897A1449A69972F170FC6C56286335570464B9FE0`. The verified full-save rollback is `C:\Users\Zemio\OneDrive\Documents\GitHub\Hytale-rollback\hywind-deploy-20260920T193703Z`. The clean full retained suite, package audit, isolated Hytale-server smoke, deployment dry run, and two deployed startup/restart cycles pass. Connected-client rendering remains the acceptance boundary; see `r066-spark-world-quad-hotfix.md`.

## R067 Spark scale, travel, and texture-isolation correction

R067 is implemented, packaged, and deployed from implementation commit `ea19efd0a6fe623c852fde2ebc977bb6e504b817`. It doubles Spark's visual width and depth to a two-block square footprint, halves travel speed from 12 m/s to 6 m/s, and increases the actual-path range from 10 m to 15 m. The resulting maximum flight time is 2.5 seconds. Damage, Mana cost, cooldown, bolt count, steering, terrain behavior, enemy penetration, Electrified ownership, and Ricochet behavior are unchanged.

The HUD screenshot was not caused by incorrect ability-item IDs: E and R still projected the correct authored `RPG_Ability_Charged_Bolt` and `RPG_Ability_Static_Field` items, and their packaged 128x128 icons passed the hash gate. R067 isolates the model animation from those icons by replacing the horizontal Spark atlas with a fresh 80x320 vertical strip and native-style Y-axis frame offsets. The old horizontal texture URI is absent from the JAR. The reported cast refusal was also resolved from the detailed trace as an authoritative `INSUFFICIENT_RESOURCE` rejection at Spark's unchanged 7 Mana cost; a later cast succeeded after Mana recovered.

The installed `Hywind.jar` SHA-256 is `50C0EACCD46F167AE9B24B010811C3A4732F7D3FAE831930E85098D7BD323B49`. The verified full-save rollback is `C:\Users\Zemio\OneDrive\Documents\GitHub\Hytale-rollback\hywind-deploy-20260920T200055Z`. The clean full retained suite, package audit, isolated Hytale-server smoke, deployment dry run, and two deployed startup/restart cycles pass. Connected-client visual and HUD-icon confirmation remains pending; see `r067-spark-scale-travel-and-icon-isolation.md`.

## R068 Spark lifecycle, animation, and expiry correction

R068 is implemented, packaged, and deployed from implementation commit `869d484d01e1d82d1edf8aed9eb92942bedb744c`. The detailed trace proved that the new cast refusal was not a Mana failure: completed variable-count Spark casts retained reservations for unselected bolts until the owner hit `OWNER_PROJECTILE_BUDGET`. Generation-zero accounting now reserves the exact deterministic three-to-five bolt count and releases the root after the final real carrier ends. A 32-cast same-owner regression verifies that active carriers and root reservations return to zero after every cast.

The static visual was caused by Spark's manually moved native carrier remaining in `Idle` while its four-frame animation was mapped only to `FlyIdle`. Both states now loop a fresh R068 animation URI, preventing stale client-resource reuse. Spark also bypasses the generic projectile-expiry line so its world quad simply disappears when the authoritative projectile terminates. Movement, damage, Mana cost, cooldown, terrain following, enemy penetration, Electrified, and Ricochet behavior are unchanged.

The installed `Hywind.jar` SHA-256 is `7A5CAD3020BF53E7626F45DDF8D91E6ED4A4B90C5F94F1676D10416741D50353`. The verified full-save rollback is `C:\Users\Zemio\OneDrive\Documents\GitHub\Hytale-rollback\hywind-deploy-20260920T202053Z`. The clean full retained suite, package audit, isolated Hytale-server smoke, deployment dry run, and two deployed startup/restart cycles pass. Connected-client animation, repeated-cast, and clean-termination confirmation remains pending; see `r068-spark-lifecycle-animation-and-expiry.md`.

## R069 Spark triple-plane, block-step, and impact presentation

R069 is implemented, packaged, and deployed from implementation commit `6602ce642a32e465d2e70b25e96bf1e5ed7910ad`. Spark's visual carrier now contains three equal-size, mutually perpendicular fullbright planes with `+X`, `+Y`, and `+Z` normals. All three planes share the same transparent blue/white vertical texture strip and synchronized four-frame 100 ms animation. A fresh model and animation URI prevent stale client-resource reuse.

Ground travel now has an explicit symmetric one-block step boundary: one-block rises and drops remain traversable, while two-block barriers and unsupported two-block drops terminate the projectile. Actual wall and Ricochet contacts emit the configured `Laser_Impact` particle, as enemy contacts already do; ordinary range/lifetime expiry and unsupported drops do not fabricate an impact. Speed, range, damage, Mana, cooldown, steering, bolt count, enemy penetration, Electrified ownership, and Ricochet count remain unchanged.

The installed `Hywind.jar` SHA-256 is `B060542E6776364A639A89C83081031FD06C62AEFFA011FD0CE33DB74090AF26`. The verified full-save rollback is `C:\Users\Zemio\OneDrive\Documents\GitHub\Hytale-rollback\hywind-deploy-20260920T205310Z`. The clean full retained suite, package audit, isolated Hytale-server smoke, deployment dry run, and two deployed startup/restart cycles pass. Connected-client visual, terrain traversal, and impact-particle confirmation remains pending; see `r069-spark-triple-plane-step-impact.md`.
